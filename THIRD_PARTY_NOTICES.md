...
| tiny-AES-c | master (vendored) | Public domain (Unlicense) | AES-256-CTR primitive for the optional `.ptm` cipher chain (gated; ON only when `-DST_ENABLE_COBB_AP_CIPHER=ON`). Vendored under `src/devices/ets/third_party/tiny_aes/`. | https://github.com/kokke/tiny-AES-c |
| bzip2 1.0.8 (decompress-only) | 1.0.8 (vendored, trimmed) | bzip2 license (BSD-style) | bzip2 inflation for the optional `.ptm` cipher chain layer 4. Vendored under `src/devices/ets/third_party/bzip2_dec/` with compress-side sources (`compress.c`, `blocksort.c`) dropped per `specs/cobb-ap3-tier3-dep-survey.md`; references from `bzlib.c` resolved via `compress_stubs.c` no-op shims. | https://sourceware.org/pub/bzip2/ |
