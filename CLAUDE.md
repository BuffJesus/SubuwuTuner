...
- `st::autotune` — v1.1 auto-tune kernel per `docs/12`. `tune_maf(axis, current_scaling, samples, opts)` returns one `CellProposal` per axis breakpoint: trimmed-mean of observed (actual_afr / commanded_afr) ratios for samples bucketed to nearest cell, scaled by `gain`, clamped to `±max_delta_pct`, with a 0..1 confidence score that ramps linearly from `min_samples_per_cell` to 3× that. `sample_passes_gates` implements the data-quality gates from `docs/12` §"Data-quality gates" — coolant ≥ 80 °C by default, IAT band, RPM/throttle rate-of-change caps, no knock in the last window, no limp mode, optional open-loop-only. Pure-domain function: no log/CSV/CLI plumbing yet (those are separable slices); 16 tests cover happy path (lean + rich), clamping (both signs), gates (cold-coolant, knock, transients, degenerate AFR), sufficiency (below min_samples leaves cell unchanged), trimmed-mean outlier rejection, and argument validation.
