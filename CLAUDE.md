...
├── tests/unit/{core,rom,defs,edit,project,transport,ecu,log,can,dbc,discover}/   (269 C++ tests; 58 Python tests under tools/defgen/tests/)
...
- **defgen polish** — ✅ `--apply-to-pack` shipped (additive merge, idempotent on a second run); ✅ non-linear `toexpr` flattening + unknown storage/endian fallbacks now surface as per-pack warnings on stderr; ✅ `ast`-based linear simplifier replaces the original regex (handles arbitrary `+/-/*/`/unary-minus compositions, so the divide-by-N and chained forms that dominate Subaru's `expression=` attributes reduce exactly — only RomRaider's `/*RSHIFT*//*LSHIFT*//*INVERSE_DIVIDE*/` hardware-shift comments are still flagged for hand-edit, since the comment changes the meaning of the surrounding expression); ✅ RomRaider `1D` scalar tables emit as `dimensions=0` (no axis required) and unresolvable-axis `2D` tables demote to scalar with a warning, so the entire VA + VB pack set now loads cleanly through `pack-info`; ✅ end-to-end against real-world XML — Merp's 332-ROM canonical pack plus all 8 VA ROMs + 18 VB ROMs round-trip through `defgen` → `pack-info` with rc=0.
