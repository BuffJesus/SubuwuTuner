...
├── tests/unit/{core,rom,defs,edit,project,transport,ecu,log,can,dbc,discover,policy}/   (280 C++ tests; 81 Python tests under tools/defgen/tests/)
...
- `st::policy` — jurisdiction-profile + linter-decision matrix per `docs/06-legal-ethics.md`. `Profile` enum (`MotorsportOnly` default, `AlbertaCa`, `EuRoadworthy`, `CaliforniaUs`) + `Action` enum (Silent / Badge / Warn / Confirm / ConfirmWithReason / Block). `emissions_action(profile)` returns `{on_save, on_flash}` per the doc's matrix; `engine_safety_on_flash(profile)` always returns Block (profile-invariant per docs/06). `profile_name`/`parse_profile` round-trip for config persistence. Pure data, no I/O — GUI + flash orchestrator consume it.
