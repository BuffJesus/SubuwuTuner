...
As of 2026-05-12: Phase 0 done. Phase 1 CLI side done. Phase 2 MVP done (persistence + undo/redo end-to-end). Phase 3 protocol framing complete (SSM read+single-byte-write+block-write, complete UDS catalog including the flashing flow, memory-by-address reads/writes, and bus gating). Phase 3 datalogger pipeline end-to-end hardware-free. Phase 4 flash orchestrator (st::flash) end-to-end hardware-free: read-full-ROM, delta detection, full erase/write/verify per-sector, dry-run, all driven through MockTransport. v1.1 auto-tune kernel started (st::autotune::tune_maf, hardware-free, per docs/12). CAN reverse-engineering toolkit replay path complete end-to-end: `st::can::Frame` + `.asc` I/O, `st::dbc::Database` parser/emitter + `decode_signal`, `st::discover::BaselineModel` + `ChangeDetector` + `.cdb` Bundle I/O, and all five replay CLI subcommands (`can-replay`, `can-diff`, `can-discover`, `can-export-dbc`, `can-decode`). C++ TOML loader resolves `extends` chains. GUI has docking, ImPlot heatmap view, 3D slice picker, welcome panel with persistent recents (per-platform config dir; dead entries shown disabled), chip-based table-header + status-bar. **353 C++ + 42 Python tests** green on MinGW g++ 15.2. Repo at `https://github.com/BuffJesus/SubuwuTuner`. Phase 1 hardware gate (real ROM, ≥20 maps from real definitions) and protocol validation waiting on user's OBDX Pro VX adapter.
