...
├── tools/defgen/                          (Python: RomRaider XML -> our TOML; clean-room facts-only; handles <base> inheritance; `loggergen.py` for SSM logger.xml -> pid TOML)
├── definitions/                           (358 defgen-generated ECU packs: 26 current-WRX from fact-only XML, 332 older Subarus from Merp; plus `pids.toml` (91 SSM datalogger PIDs))
├── fixtures/
...
├── tests/unit/{core,rom,defs,edit,project,transport,ecu,log,can,dbc,discover}/   (269 C++ tests; 71 Python tests under tools/defgen/tests/)
