...
**Phase 0 done; Phase 1 CLI side done; Phase 2 MVP done (persistence + undo/redo); Phase 3 protocol-side substantially built out** (2026-05-11). Working: `st::core`, `st::Rom` (read + write), `st::Definition` (single-file or directory loader, typed reads + writes, scaling + inverse, axis/table value extraction for 1D/2D/3D, diff, writeback), `st::edit` (Rect-scoped set/add/multiply/percent/smooth/interpolate, Snapshot, History undo/redo with branching), `st::Project` (.stune directory persistence with persisted edit history), `st::transport` (ITransport + MockTransport), `st::ecu::ssm` (A8 read + B0 write + SsmClient), `st::ecu::uds` (RDBI/WDBI/SA/DSC/ECUReset/TP/Download/Transfer/Exit — full flashing flow through MockTransport), `tools/defgen` (Python RomRaider→TOML with `<base>` inheritance). CLI: `rom-info`, `dump-axis`, `dump-table [--csv]`, `rom-diff`, `table-edit`, `project-{new,info,edit,undo,redo}`, `pack-info`, `table-list`. **172 C++ + 36 Python tests green** locally on MinGW g++ 15.2. CI: Win MSVC / macOS Apple Clang / Linux GCC / Linux Clang ASan+UBSan + Python defgen tests. Phase 5+ CAN reverse-engineering toolkit designed in `docs/14`. Remaining hardware-free work: C++ side `extends`, GUI polish stack (docking, ImPlot, nfd, theming, map editor widget) on top of the Dear ImGui + GLFW MVP, datalogger LogStream, CAN module implementation.
