...
**Phase 0 done; Phase 1 CLI side done** (2026-05-11). Working: `st::core`, `st::Rom`, `st::Definition` (TOML loader, typed reads, scaling, axis/table value extraction, per-table diff), `tools/defgen` (Python RomRaider→TOML), and CLI commands `rom-info` / `dump-axis` / `dump-table` / `rom-diff`. 65 C++ tests + 28 Python tests green locally on MinGW g++ 15.2. CI: Win MSVC / macOS Apple Clang / Linux GCC / Linux Clang ASan+UBSan + Python defgen tests. Phase 1 hardware gate (≥20 factory maps on a real stock dump) waits on a real ROM from the user's car. Hardware-free Phase 2 work (project files, editing operations) is the natural next direction.
