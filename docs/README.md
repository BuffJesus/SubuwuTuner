...
**Phase 0 done; Phase 1 CLI side done; Phase 2 MVP done; Phase 3 designed + scaffolded** (2026-05-11). Working: `st::core`, `st::Rom` (read + write), `st::Definition` (single-file or directory loader, typed reads + writes, scaling + inverse, axis/table value extraction, diff, writeback), `st::edit` (Rect-scoped set/add/multiply/percent/smooth/interpolate, Snapshot, History undo/redo with branching), `st::Project` (.stune directory persistence), `st::transport` (ITransport + MockTransport), `tools/defgen` (Python RomRaider→TOML). CLI: `rom-info`, `dump-axis`, `dump-table`, `rom-diff`, `table-edit`, `project-new`, `project-info`, `project-edit`. **109 C++ + 28 Python tests green** locally on MinGW g++ 15.2. CI: Win MSVC / macOS Apple Clang / Linux GCC / Linux Clang ASan+UBSan + Python defgen tests. Phase 1 hardware gate (≥20 factory maps on a real stock dump) waits on a real ROM from the user's car. Hardware-free work remaining: SSM/UDS clients against MockTransport with canned traces, 3D tables, definition inheritance, defgen `<base>` handling, Qt UI bring-up.
