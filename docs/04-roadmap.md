...
Heavy progress — what was sized at 4–6 weeks for the editor + IR + one backend shipped much faster. RH850 codegen for VB is now mostly shipped: all 3 IR shapes wired with a 22-primitive CallPrimitive slice covering int arithmetic, int compares, bool logic, branchless select, float arithmetic, float unary (`sqrt_float`, `flex_fuel_scale`), and float compares. Nested CallPrimitive operands and the patch-insertion layer + end-to-end flashing all gate on Phase 4 hardware validation — the float-compare path has an unverified TRFSR-direction assumption that the bench rig settles. See `docs/16-custom-features.md` for the granular current-state matrix.
...
- 🟡 RH850 codegen for VB — all 3 IR shapes wired; CallPrimitive slice covers 22 leaf-operand primitives at parity with SH-2A (int arithmetic, int compares, bool logic, branchless select, float arithmetic, `sqrt_float`, `flex_fuel_scale`, float compares). Nested CallPrimitive operands remaining; bench-rig HIL still gates first real-ECU patch (float-compare TRFSR-direction is the open verification item)
