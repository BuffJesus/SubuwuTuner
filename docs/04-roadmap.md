...
7. 🟡 **Frozen `defgen` binary** — PyInstaller spec landed at `tools/defgen/defgen.spec` (single-file build, datas=['data'], strict excludes). CI release-pipeline integration (run pyinstaller → bundle into installer) remains. End-user install still works from source if Python 3.12 is on PATH.
