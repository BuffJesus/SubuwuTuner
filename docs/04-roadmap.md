...
2. 🟡 **Cancellation invariants enforced + tested** — see `docs/08-testing-strategy.md` Tier 2a. Mid-PDU UDS / SSM cancellation must defer to PDU boundaries; crash-mid-flash must be recoverable. Spec landed in PR #1. Tests landed for the two invariants that don't need new enforcement code (crash-mid-flash recovery, resume idempotence) in `tests/unit/flash/test_cancellation_invariants.cpp`, plus a PDU-atomicity-by-construction suite in `tests/unit/ecu/test_uds_pdu_atomicity.cpp`. Remaining ⬜: extend `Flasher::execute()` to accept a cancel token (between-sector + between-PDU checks, `RequestTransferExit` on mid-sector cancel, `DiagnosticSessionControl(default)` on cancel exit), then the mid-PDU UDS / SSM cancel-deferred + session-exit-on-cancel tests slot into the placeholder block in the existing file.
