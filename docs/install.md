...
**`ap3 pull` of a large file (>5 KB) advertises wrong wire_len and hangs** — `ap3 pull /backupcksum` (33 bytes) works correctly; `ap3 pull /maps/Stage1*.ptm` (50+ KB) doesn't. The AP responds to cmd 0x21 with `wire_len = 5500` regardless of actual file size, then never sends the body. Wedges the AP until replug. Known live since 2026-06-12; under analyst investigation per `findings/handoffs/HANDOFF-to-analyst-2026-06-12-cmd21-large-file-truncation.md`. Workarounds while this is resolved: stick to `ap3 backup` for whole-vault snapshots (each file in the manifest may exhibit the same issue), or use APManager for the tune pull and then `ptm import` from disk.
