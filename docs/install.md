...
**Auto-discovering a base ROM for `ptm import`** — `subuwutuner-cli ptm import` requires the stock ROM the tune patches were authored against. Pass `--base-rom <path>` explicitly, or set `ST_PTM_BASE_ROM_DIR` to a directory containing files named `<vehicle_id>.bin` (the vehicle_id comes from the `.ptm`'s PrivateData XML — for the 2017 USDM WRX MT it's `SUBA_US_WRXM_CF_17_F`). When `--base-rom` is omitted, the importer looks at `ST_PTM_BASE_ROM_DIR/<vehicle_id>.bin` and uses it if found.
...
**~~`ap3 pull` of a large file advertises wrong wire_len and hangs~~** — **RESOLVED 2026-06-12 PM.** Root cause was a client-side bug: SubuwuTuner was sending `FileInfo2.path = "/maps/"` (directory with leading slash) in the cmd 0x20 ReadFile setup. The AP firmware expects `path = "maps/Stage1.ptm"` (full relative path including basename, no leading slash) — same convention cmd 0x22 PutFile and cmd 0x25 RemoveFile already used. After the fix (commit on top of `2ad09f7`), `ap3 pull /maps/a personal tuner-on-FA24_v3.ptm` writes a 62365-byte file byte-identical (MD5 `c83f311e4089f8941e141e8e71b38e72`) to the analyst-tool's parallel pull. Diagnosed by running the reference Python tool `findings/for-dan/ap3-toolkit/ap_pull_state.py` against the live AP and comparing its successful cmd 0x20 OUT bytes to ours.
