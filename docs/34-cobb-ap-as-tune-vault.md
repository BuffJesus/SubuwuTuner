...
**Session 3 (Blowfish OTA `.img`)** still pending — separate ~2-hour task; OTA firmware decryption isn't on the v1.0 critical path.
...
The `--allow-unmarried-ap` flag still applies regardless of the daze state.
