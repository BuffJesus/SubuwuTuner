...
3. Change the stock value (`0x0320` = 80.0°, FA20 lobe phase) to the FA24-cam-target value. A flipped-triangle lobe is geometrically a 180° crank shift = 90° cam shift. The placeholder first-guess is **~180° crank (raw `0x0A28`)** or **~90° cam** depending on which unit convention the FA20 firmware uses on this scalar; the unit isn't yet pinned down (cluster span 80°–345° is consistent with cam convention; weak inference only). The ground-truth value is whatever a publicly-circulated FA24-swap basemap supplies — byte-diff that basemap against a stock LF79103P at `0x49BA8` to read it directly.
4. Repair the calibration checksum (SubuwuTuner does this automatically via `st::flash::repair_cobb_checksum_table`).
5. Flash.
