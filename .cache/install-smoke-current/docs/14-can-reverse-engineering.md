...
**Status (updated 2026-08-21):** the **replay path is shipped** — `st::can` (Frame, `.asc` reader/writer), `st::dbc` (parser/emitter/decoder), `st::discover` (`BaselineModel`, `ChangeDetector`, `.cdb` Bundle), and the five CLI subcommands `can-replay`, `can-diff`, `can-discover --from`, `can-export-dbc`, `can-decode` are all in `main` and unit-tested. Raw **live passive capture is also shipped** as `subuwutuner-cli sniff` through the OBDX transport. It writes the v1 sniff-log format rather than `.asc`, so direct ingestion into the replay/discovery commands remains an open bridge. See `docs/13-transport.md`, `docs/24-sniff-workflows.md`, and "Implementation order" below.
...
7. ⚠ **Live mode** — OBDX raw listen-only capture is shipped as `sniff`; interactive discovery and lossless v1-sniff-log-to-`.asc`/`.cdb` ingestion remain open. Same algorithm, different input source.
8. ✅ **`can-diff`** — SavvyCAN-Discrepancies analog for users who prefer that workflow.

Each step is independently shippable and unit-testable. Items 1–6 + 8 are in
`main`; item 7 is partially shipped through the OBDX `sniff` path and still
needs the discovery-format bridge and interactive live surface (see docs/13).

A hypothetical `can-record` CLI verb is *not* shipped. Capture can route through SavvyCAN (or any tool that writes `.asc`) directly into `can-discover --from`, or through `subuwutuner-cli sniff` for OBDX listen-only raw capture. The latter requires a lossless format adapter before the `.asc` discovery commands can consume it directly.
