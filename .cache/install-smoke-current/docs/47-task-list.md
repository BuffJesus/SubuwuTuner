...
- The application is already useful without a functioning ECU: ROM viewing,
  ...
  mocks.
- Offline analysis has produced full Ghidra decompilation for EP5G, EZ1G,
  LF79100P, and A8DH101I. LF79 has 480 analyst-side verified function names;
  A8DH has public XML table labels and a sibling-variance report.
...
- `[~]` `docs/43-jtag-recovery.md` remains useful as a historical SH-2A/SH7058
  reference, but its E2-Lite target, pinout, and live-RAM steps are not yet
  validated for every SH72543-style VA ECU.
...
6. `[~]` Add a release-package smoke pass that runs the same Welcome/Demo loop
   against installed assets rather than the source-tree build layout. Install
   rules now stage docs, changelog, demo project, and synthetic log fixtures;
   isolated installed-binary validation is in progress.
...
- `[ ]` VB LHB and other RH850/VB candidates: inventory source quality first,
  then seed/analyze/decompile only canonical plaintext images.
...
4. Add provenance/confidence fields to the definition model.
5. Finish LF79 mismatch review and A8DH101I QA.
...
7. Encode the FA24 five-table guided workflow with “unverified on hardware”
   states.
8. Complete checksum byte-validation and Phase D durable-commit tests.
