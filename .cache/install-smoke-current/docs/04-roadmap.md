...
**Current hardware status (2026-08-02):** the LF79002P bench ECU is silent
after power-cycle following the round-57 real-flash drift event. The Phase 3/4
architecture remains testable with mocks and fixtures, but live validation is
blocked until qualified exact-board recovery or an exact-CID replacement donor.
Do not interpret older “hardware now in hand” or “boot validated” language
below as current ECU availability; see `docs/47-task-list.md`.
...
**Historical Phase 3 snapshot:** 🟡 hardware-actionable. Code paths were real +
unit-tested; OBDX Pro VX and COBB AccessPort V3 were in hand, and the bench rig
was described as Phase 2 boot validated. The current hardware correction below
supersedes that availability claim.

**Status correction:** the first OBDX identity/read attempt was made, but the
LF79002P ECU is now silent after power-cycle following the round-57 flash
failure. Treat the live bench path as blocked until qualified recovery or an
accepted exact-CID replacement donor; the older “boot validated” wording above
is historical. Hardware-free progress since that snapshot includes exact-CID
calibration-region binding in Flash Review/Readiness and the first Map Explorer
facets in the Tables sidebar; neither substitutes for live observed-CID proof.
...
## Phase 5 — Custom features (4–6 weeks) 🟡 design + IR + SH-2A codegen for VA shipped; RH850 codegen for VB at SH-2A parity; patch insertion + bench-rig HIL remaining
...
7. ✅ **Frozen `defgen` binary** — PyInstaller spec at `tools/defgen/defgen.spec` plus the `defgen-freeze` CI matrix job in `.github/workflows/ci.yml` (Windows / macOS / Linux runners each build + smoke-test + upload one frozen binary per platform; 14-day artifact retention). Release pipeline grabs the three artifacts and bundles each into its platform-specific installer. End-user install still works from source if Python 3.12 is on PATH.
8. ✅ **README platform-feature matrix** — table at `README.md` §"Platform feature matrix" enumerates Windows / macOS / Linux support per capability, with J2534 flagged as Windows-only.
9. ✅ **OFL font licenses in `NOTICE`** — NOTICE references Inter + JetBrains Mono under OFL-1.1; full license text reproduced in `THIRD_PARTY_NOTICES.md` §"Bundled fonts".
...
11. ✅ **Property-based tests** on the codec layer + UDS framing + UDS orchestrator state — in-tree harness at `tests/unit/_helpers/property.hpp` (deterministic seeded PRNG, no external dep). Coverage: DVI codec (`test_obdx_dvi_properties.cpp`, 8 cases), SSM framing (`test_ssm_properties.cpp`, 8 cases), native SOF/seq/CRC-16 (`test_native_properties.cpp`, 11 cases), UDS framing + NRC dispatch + robustness across the whole parser catalog (`test_uds_properties.cpp`, 17 cases), UDS state-machine sequence (`test_uds_state_machine_properties.cpp`, `e6112f6`, 80-iteration property test over random plan shapes — DSC(programming) first, CC(off) iff `silence_bus`, per-sector ordering, `dry_run` emits zero sector PDUs, between-sectors cancel emits DSC(default) last). Each parser is verified against: build/parse roundtrip, negative-response (any NRC) rejection, mismatched-echo rejection (wrong DID / wrong session / wrong counter), and adversarial-random-bytes robustness (no crash, only clean errors).
