...
## `[[calibration_region]]` — exact-CID flash allow-list

`[[calibration_region]]` is intentionally separate from
`[[writable_region]]`. The latter is a codegen/patch-insertion gate; it is not
permission for ordinary calibration writes. A calibration-region entry is
eligible for the flash preflight allow-list only when its `cid` matches the
identified ECU and its `status` is `approved`.

```toml
[[calibration_region]]
name        = "lf79002p-calibration"
cid         = "LF79002P"              # exact CID, not a family wildcard
address     = 0x00010000
length      = 0x001F0000
status      = "candidate"             # candidate | approved
provenance  = "analyst-byte-verified" # evidence source, never implied
description = "Calibration extent pending independent bench validation"
```

Required fields are `name`, exact `cid`, `address`, `length`, and non-empty
`provenance`. `status` defaults to `candidate`; only `approved` entries may be
converted by a live host into `PreflightContext::approved_write_regions`.
The loader validates positive length, address overflow, ROM bounds, and the
`candidate | approved` status vocabulary. Inheritance overrides by `(cid,
name)`. This schema does not approve a region by itself: the host must still
bind the identified ECU, source image, recovery image, journal, and current
definition evidence before a write can proceed.

At runtime, `Definition::match_info(rom)` returns the matched exact CID as
well as the identification label and offset. The host may call
`Definition::approved_calibration_regions(cid)` to obtain only approved
entries for that exact CID and convert them into
`policy::PreflightContext::approved_write_regions`. A source-ROM match is
local evidence only; the live observed CID remains a separate mandatory
preflight proof.
