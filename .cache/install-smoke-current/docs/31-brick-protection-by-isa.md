...
> **Current recovery status (2026-08-02):** the LF79002P bench ECU is silent
> after power-cycle. The historical E2-Lite procedure below is not an
> approved recovery path for the current ECU until exact MCU/board/debug
> interface and vendor-supported probe evidence are confirmed. Use
> `docs/43-jtag-recovery.md` as a quarantined historical reference only; the
> immediate project path is `docs/50-replacement-ecu-intake.md`. No OBD retry
> is authorized.

Operational recipe (see `docs/43-jtag-recovery.md` for the full
step-by-step):

1. **Use only a verified probe/recovery service for the exact ECU.** The
   former Renesas E2-Lite recommendation is withdrawn. E200F/E10A-class
   SH-2A H-UDI research is tracked in `docs/49-diy-sh2a-recovery-probe.md`,
   but target support, board access, and the complete recovery sequence remain
   unvalidated for this ECU. JTAG operates below the FCU protocol layer in
   principle, but that does not prove a usable board connection or target
   procedure.
...
exists for this path but is unconfirmed for SH72531/SH72543-class
chips in FA20DIT ECUs. Use it only as an older SH7058 research branch;
do not default to E2-Lite or any generic JTAG tool for the current ECU.
...
it inappropriate for a tuning tool. The current bench rig does not have a
