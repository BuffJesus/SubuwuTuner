...
## Current hardware status (2026-08-02)

The offline ROM, definition, project, diff, reverse-engineering, and Map
Explorer surfaces remain available. The LF79002P bench ECU is currently silent
after the round-57 flash incident, so live ECU validation and any further write
attempts are blocked. The GUI now selects approved exact-CID calibration
regions for offline Flash Review, but live observed-CID proof remains required.
The previously discussed Renesas E2-Lite path is not approved for the SH72543-
style target; recovery requires either a verified exact-board service/probe path
or an exact-CID replacement ECU. See [`docs/43`](43-jtag-recovery.md),
[`docs/47`](47-task-list.md), [`docs/49`](49-diy-sh2a-recovery-probe.md), and
[`docs/50`](50-replacement-ecu-intake.md).
