...
> **Execution status (2026-08-02): BLOCKED.** This runbook is retained as the
> post-recovery validation sequence, but the current LF79002P ECU is silent
> after power-cycle. Step 0 cannot pass, and Steps 5 onward must not be run on
> the current ECU. Recovery hardware and the historical E2-Lite reference are
> not validated for this exact SH72543-style board. Resume this runbook only
> with a qualified recovery result or an accepted exact-CID replacement; use
> `docs/50-replacement-ecu-intake.md` for the latter.
...
```bash
# Build the synthetic-delta target image, then turn it into a plan.
$CLI flash-delta /tmp/bench-rig-rom.bin /tmp/bench-rig-target.bin \
  --layout fa-dit-sh2a-2mb --output /tmp/bench-rig.plan.toml
# Read the plan before sending it -- it is hand-editable on purpose.
$CLI flash-plan-info /tmp/bench-rig.plan.toml
# Dry-run against a recorded trace first (no transport, no bytes on the bus):
$CLI flash-apply --plan /tmp/bench-rig.plan.toml --trace /tmp/bench-rig.uds
# If the sequence looks right, run for real. --transport requires --confirm.
$CLI flash-apply --plan /tmp/bench-rig.plan.toml \
  --transport obdx --device COM3 \
  --journal /tmp/bench-rig.journal.toml \
  --confirm --reason "bench rig step 5 first deliberate write"
```

> `project-flash` is a preview verb and takes no transport flags. Live
> writes go through `flash-apply --plan`. Keep the `--journal` path on
> every real write — step 6 needs it to resume.
...
**Fail (worst case):** ECU doesn't boot. Power-cycle once for diagnosis. If
it remains silent, stop active OBD attempts, preserve the image/logs, and use
the exact-board recovery decision in `docs/43-jtag-recovery.md` or the donor
intake in `docs/50-replacement-ecu-intake.md`. The historical SH7058/E2-Lite
and mode-pin notes are not validated for the current SH72543-style board.

> **Recovery-path correction:** if the ECU remains silent after one diagnostic
> power-cycle, do not follow the historical E2-Lite or mode-pin suggestion in
> the Step 5 failure paragraph. Preserve the image and logs, stop OBD writes,
> and use the exact-board decision in `docs/43-jtag-recovery.md` or the donor
> intake in `docs/50-replacement-ecu-intake.md`.
...
$CLI flash-apply --plan /tmp/bench-rig.plan.toml \
  --transport obdx --device COM3 \
  --journal /tmp/bench-rig.journal.toml \
  --confirm --reason "power-loss inject"
...
# Resume from the journal the interrupted run left behind:
$CLI flash-resume /tmp/bench-rig.plan.toml /tmp/bench-rig.journal.toml \
  --output /tmp/bench-rig.resumed.plan.toml
$CLI flash-apply --plan /tmp/bench-rig.resumed.plan.toml \
  --transport obdx --device COM3 \
  --journal /tmp/bench-rig.journal2.toml \
  --confirm --reason "resume after power-loss inject"
