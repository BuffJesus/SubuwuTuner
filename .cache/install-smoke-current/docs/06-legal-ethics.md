...
The project's response is **Path B** (`docs/17-data-distribution-policy.md`): the public repo carries the tool plus community-sourced definition data. The initial Path B commit was conservative — it excluded the **VA WRX (2015–2021)** and **VB WRX (2022+)** platforms because the only available definition data at that point had upstream §1201 problems. That changed 2026-05-19 when forum-sourced per-CID RomRaider XMLs for both platforms surfaced clean of §1201 concerns; per `docs/17` §4 acceptance criteria, those packs now ship in `definitions/impreza/` alongside the EJ-era WRX/STI work (7 LF VA + 18 LHB VB CIDs covered). One user-master CID (LF9D012H) remains unshipped because no per-CID XML has been sourced for it. Users still bring their own packs for any platform / CID outside the in-tree per-CID set, via `tools/defgen/` on a sourced RomRaider XML, hardware capture from a ROM they legally possess, or community import.
