...
| **CLI** | `subuwutuner-cli feature-compile <stmod> --def <pack> [--arch sh2a\|rh850] [--format hex\|toml\|raw\|stmod] [--output <file>] [--validate-only]`; `feature-flash <stmod> --def <pack> --source <ROM>` inserts a patch, repairs checksums, and emits a policy-gated `FlashPlan`. Plus `dump-ir`, `lint-graph`, `lint-ir`. `--format=stmod` bundles graph + patch in a single TOML; `--validate-only` runs parse + lower + compile and exits 0/non-zero without producing output — for CI / pre-commit hooks. | ✅ |
...
| **Patch insertion** | Finding free RAM, writing the hook table, splicing into existing interrupt vectors. Host-side implementation + synthetic tests are shipped; real-ECU validation remains bench-gated. | 🟡 |
| **Flashing** | `feature-flash` emits the standard plan; live writing delegates to the explicitly confirmed `flash-apply` path. End-to-end ECU validation remains bench-gated. | 🟡 |
...
| **HPFP control** | The HPFP-driving intake cam lobe is a triangle shape — and between FA20 and FA24 **the triangle got flipped**. The FA20 ECU's commanded HPFP solenoid energize angle lands on the FA24 lobe's *downstroke* instead of its upstroke. The pump runs max duty against the commanded target, climbs to ~4000 psi (the pump can in fact exceed FA20 targets — the failure isn't capacity, it's phase), and the ECU's ~4200 psi safety cutoff kills the engine. This is a **calibration on HPFP command crank angle**, not on duty / pressure. Per a publicly-circulated FA24-swap recipe, an open-source competing tuner is the only currently-tunable platform that exposes this surface; tunes done on the mainstream commercial platforms must run FA20 cams to dodge the issue. | **Static table edit on `Fuel - Timing - HPFP - Base Offset`** — a single u16 scalar (deg/10 scaling) that the firmware applies as a constant additive phase bias to the commanded HPFP energize angle. Static edit is the entire fix (see §"Static-edit fix" below). Note: rescaling `hpfp_duty_*` / `hpfp_target_pressure_*` tables does **not** fix this — it only changes the magnitude the wrong-stroke command is fighting toward. |
...
3. The HPFP cam-lobe phase mismatch — the FA24's flipped triangle lobe — fixed by a static edit on the `Fuel - Timing - HPFP - Base Offset` scalar in the FA20 ECU's calibration. The earlier framings (including past versions of this doc) that treated this as a pump-capacity / pressure-target problem are wrong: the FA24 pump readily exceeds FA20-commanded pressures — it just gets driven on the wrong stroke.

**Implications for SubuwuTuner's swap-support story:**

- The infrastructure to support an FA24-into-VA swap already exists in v1.0 (`st::edit`, `Definition` schema with `[[table]]` entries, v1.1 MAF autotune kernel). No runtime patch is required for the HPFP fix.
- What's missing is **firmware RE work** — finding the FA20 ECU address for the HPFP Base Offset scalar (resolved on LF79103P at `0x49BA8`; sister VA CIDs need the same locate) and adding it to the VA pack. That's a definition-pack contribution, not a code change.
- The custom-features designer is the right tool for **3rd-party sensor integration** patterns the swap user might pursue separately (aftermarket fuel pressure sensor → CAN bus → override decision). The `fa24-hpfp-phase-offset.stmod` sample exists as an illustrative runtime path — useful as a template if someone wants RPM/load-dependent phase tuning that a single scalar can't express, but **not the actual FA24 swap fix** (the static edit is). The earlier samples `fa24-hpfp-clamp.stmod`, `fa24-aux-pressure-clamp.stmod`, and `fa24-bernoulli-comp.stmod` were drafted under the wrong premise that the FA24 pump can't reach FA20 targets — useful as template runtime-clamp / Bernoulli-PW shapes for other applications, but not the actual fix for this swap.

### Static-edit fix (the actual FA24 swap HPFP fix)

The entire HPFP fix on a SubuwuTuner pack that has the scalar mapped is:

1. Open the LF79103P pack in SubuwuTuner.
2. Locate `Fuel - Timing - HPFP - Base Offset` at flash address `0x49BA8` (u16 big-endian, deg/10 scaling).
3. Change the stock value (`0x0320` = 80.0°, FA20 lobe phase) to the FA24-cam-target value. A flipped-triangle lobe is geometrically a 180° crank shift = 90° cam shift. The placeholder first-guess is **~180° crank (raw `0x0A28`)** or **~90° cam (raw `0x06A4`)** depending on which unit convention the FA20 firmware uses on this scalar; the unit isn't yet pinned down (cluster span 80°–345° is consistent with cam convention; weak inference only). The ground-truth value is whatever a publicly-circulated FA24-swap basemap supplies — byte-diff that basemap against a stock LF79103P at `0x49BA8` to read it directly.
4. Repair the calibration checksum (SubuwuTuner's flash pipeline applies the appropriate repair automatically via `st::flash::apply_checksum_repair`, dispatched by the pack's declared `ChecksumKind`).
5. Flash.

This is a calibration-table change — same shape as any other Subaru calibration edit. No runtime patch, no SH-2A codegen, no `feature_patch` work involved. Per evidence from the firmware layout (`0x49BA8` is prefixed to a 16-row PW table at `0x49BAA` in the same descriptor block) and the surface's documented role ("base offset, or phasing, of control"), Base Offset behaves as a constant additive phase bias on the commanded HPFP energize angle — a static edit produces a constant phase shift, which is exactly what's needed to undo the flipped-lobe geometry.
...
2. **Basemap** — "Yes, load a `.bin`" opens an NFD file picker, size-checks against the project's source ROM, and reads the 5 workflow tables from the basemap. Step 2 defaults to **Yes** for safety, but the defaults branch is now a complete standalone path: it writes a community swap-basemap vendor's actual values for every workflow table, including per-cell presets for the AVCS Cam Target (10×16) and the HPFP Phase Transfer Curve (16-element 1-D) — both extracted from a community swap-basemap vendor's basemap. Defaults and basemap branches produce identical bytes for the AVCS, HPFP, and Injector Mult tables; the only place they can diverge is the Engine Displacement scalar (basemap captures whatever the .bin holds; defaults force 2.4 L).
...
| Table id | Default op | Default arg | Needs Keep-FA24-cams | a community swap-basemap vendor rationale |
|---|---|---|---|---|
| `engine_displacement` | `set_cells` | `2.4` | no — applies for every strategy | a community swap-basemap vendor writes 2.4 L exactly. |
| `fuel_timing_hpfp_phase_transfer_curve` | per-cell preset | `kNtmHpfpPhaseTransferCurve` (16) | yes | 16-element 1-D uint8 table at canon 0x49B98 (X-axis at 0x40E04). Ghidra-resolved 2026-06-09 (see `findings/decompile/lf79103p/HPFP_0x49BA0_RESOLVED.md`); supersedes the earlier 4-byte uint32 placeholder. a community swap-basemap vendor tunes indices 6..10 to smooth the descent through the transition zone; the preset writes the full 16-byte pattern (unchanged plateaus at both ends, tuned middle). |
| `avcs_intake_barometric_multiplier_low_intake_cam_target_tgv_closed` | per-cell preset | `kNtmAvcsCamTargetTgvClosed` (10×16) | yes | a community swap-basemap vendor does a structured 2D retune (cells span 5°..30°). The defaults branch now writes a community swap-basemap vendor's exact 10×16 array (extracted from their basemap 2026-06-09). Identical to the basemap-load path for this table. |
| `avcs_intake_barometric_multiplier_high_intake_cam_target_tgv_closed` | per-cell preset | `kNtmAvcsCamTargetTgvClosed` (10×16) | yes | a community swap-basemap vendor keeps Baro Low == Baro High at every cell — shares the same preset. |
| `fuel_injectors_pulse_injector_mult_table` | `multiply_cells` | `1.43` | yes | a community swap-basemap vendor multiplies every cell uniformly by ~1.43 (not 1.18 — that was the pre-a community swap-basemap vendor-data guess). |
...
| **Free open-source competing tuner** | Stock FA20 ECU; full factory table catalog; **HPFP command-angle phase-offset exposed** (the only currently-tunable platform that does); custom-features designer can do cam-trigger signal interpretation in software if the RS Motors hardware kit isn't installed; analyticals for FA24 cam profile (LSA, overlap) | The path that lets FA24 cams stay installed. Recent publicly-circulated FA24-swap basemaps target this platform. |
| **Another commercial tuning tool** | Stock FA20 ECU; **does not expose HPFP command-angle phase calibration** — FA24-cam installs run the pump out of phase, hit the ~4200 psi safety cutoff, and the engine shuts down. Must run FA20 cams. | Workable for FA20-cam swaps only. FA24 cams force a platform change to the open-source competing tuner. |
...
- `st::edit` also covers the HPFP Base Offset scalar (the actual FA24 swap fix, see §"Static-edit fix" above). Resolved on LF79103P at `0x49BA8`; sister VA CIDs need the same locate.
- v1.1 MAF autotune kernel covers the air-mass rescaling for the larger displacement
- The custom-features designer (`src/feature` + SH-2A codegen) covers the **3rd-party-sensor-integration pattern** — the same shape as the differential-fuel-pressure compensation that mainstream commercial flashers ship (Bernoulli-based injector-scale correction over an aftermarket fuel-pressure sensor). It also serves as the runtime path for RPM/load-dependent HPFP phase tuning if a single Base Offset scalar turns out to be insufficient for some downstream user; that path is illustrated by `fixtures/samples/fa24-hpfp-phase-offset.stmod` but is **not the actual FA24 swap fix** — the static table edit is.

What's missing is **firmware-RE work**: locating the HPFP Base Offset address on the sister VA CIDs and resolving the rest of the VE / AVCS / fueling table addresses. That's a definition-pack contribution, not a code change. Once those addresses are in the VA pack, FA24-cam swaps are supported via a static table edit — no runtime patch needed.
