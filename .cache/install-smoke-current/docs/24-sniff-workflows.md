...
The replay/discovery CAN toolkit in `docs/14-can-reverse-engineering.md`
currently consumes `.asc`, not this v1 sniff-log format. Preserve the original
log and use a lossless converter once implemented; do not hand-edit timestamps,
CAN IDs, or payloads. Direct v1-log ingestion is an explicit tooling gap.

For the focused factory-style VA WRX/STI drivetrain research playbook, see
`docs/55-va-sti-dccd-integration.md`. Its first capture must be unfiltered:
DCCD, VDC/ABS, steering-angle, BIU, and combination-meter traffic will not be
confined to `0x7E0/0x7E8`. Begin with passive stock-STI and recipient-WRX
baselines; do not inject or replay frames on a live vehicle during discovery.
