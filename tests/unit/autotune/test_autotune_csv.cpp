...
TEST_CASE("read_maf_samples_csv: required columns populate fields",
          "[autotune][csv]") {
...
TEST_CASE("read_maf_samples_csv: missing required column -> error",
    ...
    // Drop iat_c.
    std::string text =
        "maf_voltage,actual_afr,commanded_afr,rpm,throttle_pct,coolant_c\n"
...
TEST_CASE("read_maf_samples_csv: column order is irrelevant",
          "[autotune][csv]") {
    // Shuffle columns.
    std::string text =
        "iat_c,rpm,maf_voltage,commanded_afr,coolant_c,actual_afr,throttle_pct\n"
...
TEST_CASE("read_maf_samples_csv: case-insensitive header names",
          "[autotune][csv]") {
    std::string text =
        "MAF_Voltage,Actual_AFR,Commanded_AFR,RPM,Throttle_PCT,Coolant_C,IAT_C\n"
...
TEST_CASE("read_maf_samples_csv: blank lines and # comments skipped",
          "[autotune][csv]") {
    std::string text =
        "# header comment\n"
        "\n"
...
TEST_CASE("read_maf_samples_csv: CRLF tolerated",
          "[autotune][csv]") {
    std::string text(kMinimalHeader);
...
TEST_CASE("read_maf_samples_csv: optional booleans",
          "[autotune][csv]") {
    std::string text =
        "maf_voltage,actual_afr,commanded_afr,rpm,throttle_pct,coolant_c,iat_c,"
...
TEST_CASE("read_maf_samples_csv: forward-diff rates use time_ms",
          "[autotune][csv]") {
    std::string text =
...
TEST_CASE("read_maf_samples_csv: zero or negative dt clamps rates to 0",
          "[autotune][csv]") {
    std::string text =
        "time_ms,maf_voltage,actual_afr,commanded_afr,rpm,throttle_pct,coolant_c,iat_c\n"
...
TEST_CASE("read_maf_samples_csv: knock_in_window covers prior 250 ms",
          "[autotune][csv]") {
...
TEST_CASE("read_maf_samples_csv: unknown columns are tolerated",
          "[autotune][csv]") {
    std::string text =
        "maf_voltage,actual_afr,commanded_afr,rpm,throttle_pct,coolant_c,iat_c,"
...
TEST_CASE("read_maf_samples_csv: integrates with tune_maf end-to-end",
          "[autotune][csv]") {
