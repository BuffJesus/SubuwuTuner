...
TEST_CASE("lint_maf_proposal: clean proposal -> no violations", "[autotune][lint]") {
...
TEST_CASE("lint_maf_proposal: monotonic check ignores tiny FP noise", "[autotune][lint]") {
...
TEST_CASE("lint_maf_proposal: step discontinuity flagged", "[autotune][lint]") {
    std::vector<double> axis{1.0, 2.0, 3.0};
    ...
    bool found = false;
    for (auto const &x : v) {
        if (x.kind == at::LintViolationKind::StepDiscontinuity && x.cell_index == 1) {
            found = true;
...
TEST_CASE("lint_maf_proposal: enforce flags can disable each check", "[autotune][lint]") {
    std::vector<double> axis{1.0, 2.0, 3.0};
    std::vector<double> current{1.0, 2.0, 3.0};
...
TEST_CASE("lint_maf_proposal: empty / single-cell proposals are clean", "[autotune][lint]") {
...
TEST_CASE("lint_kind_name: stable labels for each kind", "[autotune][lint]") {
    CHECK(std::string{at::lint_kind_name(at::LintViolationKind::NonMonotonic)} == "non-monotonic");
    CHECK(std::string{at::lint_kind_name(at::LintViolationKind::StepDiscontinuity)} ==
          "step discontinuity");
}
...
TEST_CASE("lint_knock_proposal: smooth surface -> no violations", "[autotune][lint][knock]") {
...
TEST_CASE("lint_knock_proposal: enforce flag disables the check", "[autotune][lint][knock]") {
...
TEST_CASE("lint_knock_proposal: threshold is configurable", "[autotune][lint][knock]") {
    std::vector<double> rpm_axis{2000.0, 3000.0};
    std::vector<double> load_axis{2.5};
...
TEST_CASE("lint_knock_proposal: empty result is clean", "[autotune][lint][knock]") {
