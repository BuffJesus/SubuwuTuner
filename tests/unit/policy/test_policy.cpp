...
TEST_CASE("engine_safety_on_flash always Blocks regardless of profile",
          "[policy][engine_safety]") {
...
TEST_CASE("profile_name <-> parse_profile round-trip", "[policy][serde]") {
    for (auto p : {Profile::MotorsportOnly, Profile::AlbertaCa,
                   Profile::EuRoadworthy, Profile::CaliforniaUs}) {
...
TEST_CASE("Action ordering supports `take stricter of two`",
          "[policy][action]") {
