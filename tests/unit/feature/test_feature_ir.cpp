...
st::feature::Node make_hook_node(
    char const *kind,
    ...
    for (auto const &[name, type] : outs) {
        n.pins.push_back(st::feature::Pin{
            next++, name, type, st::feature::PinDirection::Output, ""});
    }
    for (auto const &[name, type] : ins) {
        n.pins.push_back(st::feature::Pin{
            next++, name, type, st::feature::PinDirection::Input, ""});
    }
    return n;
}

st::feature::Node make_primitive_node(
    char const *kind,
    std::vector<std::pair<char const *, st::feature::PinType>> ins,
    std::vector<std::pair<char const *, st::feature::PinType>> outs) {
    st::feature::Node n;
...
TEST_CASE("ir::lower on an empty graph returns an empty module",
          "[feature][ir]") {
    st::feature::Graph g;
...
TEST_CASE("ir::lower emits LoadHookInput → CallPrimitive → "
    ...
    auto const src = g.add_node(make_hook_node(
        "hook.read_rpm",
        {{"rpm", st::feature::PinType::Float}}, {}));
    auto const mid = g.add_node(make_primitive_node(
        "primitive.passthrough",
        {{"in",  st::feature::PinType::Float}},
        {{"out", st::feature::PinType::Float}}));
    auto const snk = g.add_node(make_hook_node(
        "hook.after_fuel_calc",
        {},
        {{"Override fuel PW", st::feature::PinType::Float}}));
    ...
    REQUIRE(m->instructions[1].operands[0]
            == m->instructions[0].result_id);
    ...
    REQUIRE(m->instructions[2].operands[0]
            == m->instructions[1].result_id);
}

TEST_CASE("ir::lower emits LoadConstant for a defaulted input pin",
          "[feature][ir]") {
    ...
    cmp.label = "compare";
    cmp.pins.push_back(st::feature::Pin{
    ...
    REQUIRE(m->instructions[2].operands[0]
            == m->instructions[0].result_id);
    REQUIRE(m->instructions[2].operands[1]
            == m->instructions[1].result_id);
...
TEST_CASE("ir::lower's operand_pin_names tracks pin reordering",
          "[feature][ir]") {
    ...
    st::feature::Graph g;
    auto const src_a = g.add_node(make_hook_node(
        "hook.read_rpm",
        {{"rpm_a", st::feature::PinType::Float}}, {}));
    auto const src_b = g.add_node(make_hook_node(
        "hook.read_rpm",
        {{"rpm_b", st::feature::PinType::Float}}, {}));
    st::feature::Node sub;
    sub.kind  = "primitive.subtract_float";
    sub.label = "sub";
    // Pin order: b, a, out — reversed from the canonical declaration.
    sub.pins.push_back(st::feature::Pin{
        0, "b", st::feature::PinType::Float,
        st::feature::PinDirection::Input,  ""});
    sub.pins.push_back(st::feature::Pin{
        1, "a", st::feature::PinType::Float,
        st::feature::PinDirection::Input,  ""});
    sub.pins.push_back(st::feature::Pin{
...
TEST_CASE("ir::lower fails when an Input pin has no driver and no default",
          "[feature][ir]") {
    st::feature::Graph g;
    g.add_node(make_primitive_node(
        "primitive.compare_gt",
        {{"a", st::feature::PinType::Float},
         {"b", st::feature::PinType::Float}},
...
TEST_CASE("ir::lower refuses a graph that fails validate()",
          "[feature][ir]") {
    // Two-primitive cycle. Graph::validate flags this; ir::lower
    // surfaces the same failure rather than producing garbage IR.
    st::feature::Graph g;
    auto const a = g.add_node(make_primitive_node(
        "primitive.a", {{"in", st::feature::PinType::Float}},
                        {{"out", st::feature::PinType::Float}}));
    auto const b = g.add_node(make_primitive_node(
        "primitive.b", {{"in", st::feature::PinType::Float}},
                        {{"out", st::feature::PinType::Float}}));
...
TEST_CASE("ir::dump produces a stable human-readable transcript",
    ...
    REQUIRE(text.find("@after_fuel_calc.Override fuel PW")
            != std::string::npos);
}

TEST_CASE("ir::lint passes on a clean module under budget",
          "[feature][ir][lint]") {
    st::feature::Graph g;
    auto const src = g.add_node(make_hook_node(
        "hook.read_rpm", {{"rpm", st::feature::PinType::Float}}, {}));
...
TEST_CASE("ir::lint flags an RT-budget overrun",
          "[feature][ir][lint]") {
    st::feature::Graph g;
    auto const src = g.add_node(make_hook_node(
        "hook.read_rpm", {{"rpm", st::feature::PinType::Float}}, {}));
...
TEST_CASE("ir::lint flags duplicate StoreHookOutput",
    ...
    st::feature::Graph g;
    auto const src_a = g.add_node(make_hook_node(
        "hook.read_rpm", {{"rpm_a", st::feature::PinType::Float}}, {}));
    auto const src_b = g.add_node(make_hook_node(
        "hook.read_rpm", {{"rpm_b", st::feature::PinType::Float}}, {}));
    st::feature::Node snk;
    snk.kind  = "hook.after_fuel_calc";
    snk.label = "after_fuel_calc";
    snk.is_phase_break = true;
    snk.pins.push_back(st::feature::Pin{
        0, "Override fuel PW", st::feature::PinType::Float,
        st::feature::PinDirection::Input, ""});
    snk.pins.push_back(st::feature::Pin{
        1, "Override fuel PW", st::feature::PinType::Float,
        st::feature::PinDirection::Input, ""});
