...
TEST_CASE("Baseline classifies a byte that never changes as Stable",
          "[discover][baseline]") {
...
TEST_CASE("Baseline classifies a small-set byte as Stable",
          "[discover][baseline]") {
    // Byte 0 alternates between three values — within the default
    // stable_max_distinct of 4 → Stable.
    std::vector<std::uint8_t> const sequence{0x10, 0x20, 0x30,
                                              0x10, 0x20, 0x30,
                                              0x10, 0x20, 0x30,
                                              0x10};
    std::vector<can_ns::Frame> frames;
    for (std::size_t i = 0; i < sequence.size(); ++i) {
        frames.push_back(make(static_cast<std::int64_t>(i * 1'000'000),
                              0x200, {sequence[i]}));
    }
    auto const model = discover_ns::build_baseline(frames);
...
TEST_CASE("Baseline classifies a strictly-incrementing byte as Cyclic",
          "[discover][baseline]") {
...
TEST_CASE("Baseline classifies an unstructured byte as Noisy",
          "[discover][baseline]") {
    // A made-up sequence with no clear pattern AND many distinct values.
    std::vector<std::uint8_t> const seq{
        0x10, 0xFF, 0x22, 0x05, 0xAB, 0x71, 0x14, 0xEE,
...
TEST_CASE("Baseline computes frequency from the capture duration",
          "[discover][baseline]") {
...
TEST_CASE("ChangeDetector emits a NewId event for an unseen ID",
          "[discover][change]") {
...
TEST_CASE("ChangeDetector flags a stable-byte deviation",
          "[discover][change]") {
...
TEST_CASE("ChangeDetector ignores a cyclic byte at watch time",
          "[discover][change]") {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        watch_frames.push_back(make(1'000'000'000 + i * 1'000'000,
                                     0x300, {v, 0x00}));
    }
...
TEST_CASE("ChangeDetector coalesces multi-byte changes in one frame",
          "[discover][change]") {
    // Baseline: bytes 0+1 always 0x00. Watch: both flip in the same
    // frame. One event with two byte indices.
    std::vector<can_ns::Frame> baseline_frames;
    for (int i = 0; i < 10; ++i) {
        baseline_frames.push_back(make(i * 1'000'000, 0x150,
                                       {0x00, 0x00, 0x00}));
    }
    auto const model = discover_ns::build_baseline(baseline_frames);
    ...
    auto const events = discover_ns::detect_changes(model, watch_frames);
    REQUIRE(events.size() == 1);
    REQUIRE(events[0].changed_byte_indices
...
TEST_CASE("ChangeDetector emits a NewId only on first sighting",
          "[discover][change]") {
...
TEST_CASE("ChangeDetector with flag_noisy_deviations on flags noisy deviations",
    ...
    std::vector<can_ns::Frame> baseline_frames;
    for (std::size_t i = 0; i < seq.size(); ++i) {
        baseline_frames.push_back(make(static_cast<std::int64_t>(i * 1'000'000),
                                       0x400, {seq[i]}));
    }
    auto const model = discover_ns::build_baseline(baseline_frames);
    auto const *id   = model.find(can_ns::BusId::Hs, 0x400);
    ...
    auto const off = discover_ns::detect_changes(
        model, std::vector<can_ns::Frame>{
                   make(1'000'000'000, 0x400, {static_cast<std::uint8_t>(
                                                  id->mode_value[0] + 1U)})});
    REQUIRE(off.empty());
...
discover_ns::BaselineModel small_baseline() {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        auto const noisy = static_cast<std::uint8_t>(
            (i * 73 + 17) & 0xFF);
        frames.push_back(make(static_cast<std::int64_t>(i * 3'125'000),
                              0x140,
                              {0x00, cyc, stab2, noisy}));
    }
...
TEST_CASE("Bundle round-trips through write_cdb_string + parse_cdb_string",
          "[discover][cdb]") {
...
TEST_CASE("Bundle round-trips via the filesystem (read_cdb / write_cdb)",
          "[discover][cdb][file]") {
    auto const tmp = std::filesystem::temp_directory_path()
                    / ("st_cdb_test_"
                       + std::to_string(std::random_device{}()) + ".cdb");
...
TEST_CASE("parse_cdb_string rejects an unknown event kind",
          "[discover][cdb][error]") {
...
TEST_CASE("parse_cdb_string rejects a malformed change event",
          "[discover][cdb][error]") {
...
TEST_CASE("read_cdb errors clearly on a missing file",
          "[discover][cdb][file][error]") {
