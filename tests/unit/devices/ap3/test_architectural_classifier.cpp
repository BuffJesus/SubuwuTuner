...
TEST_CASE("classifier: LF79103P dead-fill region (0x078bc..0x10000) maps to TunerAddition",
    ...
    using L = st::devices::ap3::Layer;
    REQUIRE(st::devices::ap3::classify(map, 0x078BC) == L::TunerAddition);
    REQUIRE(st::devices::ap3::classify(map, 0x08000) == L::TunerAddition);
    REQUIRE(st::devices::ap3::classify(map, 0x0C000) == L::TunerAddition);
    REQUIRE(st::devices::ap3::classify(map, 0x0FFFF) == L::TunerAddition);
    // 0x10000 is the first byte AFTER the dead-fill region.
    REQUIRE(st::devices::ap3::classify(map, 0x10000) != L::TunerAddition);
}
...
TEST_CASE("classifier: LF79103P primary code region (0x60000..0x80000)",
    ...
    auto const &map = st::devices::ap3::default_lf79103p_layer_map();
    using L = st::devices::ap3::Layer;
    REQUIRE(st::devices::ap3::classify(map, 0x60000) == L::PrimaryCode);
    REQUIRE(st::devices::ap3::classify(map, 0x7FFFF) == L::PrimaryCode);
    REQUIRE(st::devices::ap3::classify(map, 0x80000) != L::PrimaryCode);
}
...
TEST_CASE("classifier: LF79103P secondary code region (0xC0000..0xD0000)",
    ...
    auto const &map = st::devices::ap3::default_lf79103p_layer_map();
    using L = st::devices::ap3::Layer;
    REQUIRE(st::devices::ap3::classify(map, 0xC0000) == L::SecondaryCode);
    REQUIRE(st::devices::ap3::classify(map, 0xCFFFF) == L::SecondaryCode);
    REQUIRE(st::devices::ap3::classify(map, 0xD0000) != L::SecondaryCode);
}
...
TEST_CASE("classifier: LF79103P main calibration + UDS retarget",
    ...
    auto const &map = st::devices::ap3::default_lf79103p_layer_map();
    using L = st::devices::ap3::Layer;
    REQUIRE(st::devices::ap3::classify(map, 0x12000) == L::MainCalibration);
    REQUIRE(st::devices::ap3::classify(map, 0x14100) == L::MainCalibration);
    REQUIRE(st::devices::ap3::classify(map, 0x1FF100) == L::UdsDispatchRetarget);
    REQUIRE(st::devices::ap3::classify(map, 0x1FFFE0) == L::RomTail);
}
...
TEST_CASE("classifier: addresses outside every range fall to Uncharacterized",
    ...
    REQUIRE(st::devices::ap3::classify(map, 0x10000) == L::Uncharacterized);
    REQUIRE(st::devices::ap3::classify(map, 0x11FFF) == L::Uncharacterized);
    // 0x200000 is past the 2 MB ROM end.
    REQUIRE(st::devices::ap3::classify(map, 0x200000) == L::Uncharacterized);
    REQUIRE(st::devices::ap3::classify(map, 0xFFFFFFFFU) == L::Uncharacterized);
}
...
TEST_CASE("classifier: classify_patches preserves input order",
    ...
    auto cls =
        st::devices::ap3::classify_patches(st::devices::ap3::default_lf79103p_layer_map(),
                                           offsets, lengths);
    REQUIRE(cls.size() == 4);
    using L = st::devices::ap3::Layer;
...
TEST_CASE("classifier: summarize aggregates and sorts by total_bytes",
    ...
    auto cls =
        st::devices::ap3::classify_patches(st::devices::ap3::default_lf79103p_layer_map(),
                                           offsets, lengths);
    auto summary = st::devices::ap3::summarize(cls);
...
TEST_CASE("classifier: summarize on empty input returns empty",
          "[devices][ap3][classifier]") {
    auto summary = st::devices::ap3::summarize({});
...
TEST_CASE("classifier: layer_label returns non-empty for every enumerator",
    ...
    for (auto layer : {L::HeaderBootSecurity, L::EarlyCalTables, L::TunerAddition,
        ...
        REQUIRE_FALSE(st::devices::ap3::layer_label(layer).empty());
    }
}
