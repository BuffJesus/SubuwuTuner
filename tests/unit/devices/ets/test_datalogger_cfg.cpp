...
// st::devices::ets::decode_datalogger_cfg / encode_datalogger_cfg.
// Format per RE4 (findings/re-2026-06-12-pm/).

#include "st/devices/ets/datalogger_cfg.hpp"
...
TEST_CASE("datalogger_cfg: decode canonical CRLF preset",
    ...
    auto r = st::devices::ets::decode_datalogger_cfg(cfg);
    REQUIRE(r.has_value());
...
TEST_CASE("datalogger_cfg: encode emits CRLF + AP-canonical key order",
          "[devices][ap3][datalogger_cfg]") {
    st::devices::ets::DataloggerPreset p;
    p.name = "WideOpen";
    p.description = "track day cluster";
    p.log_mon_list = {"rpm", "boost", "afr"};
    auto const text = st::devices::ets::encode_datalogger_cfg(p);
...
TEST_CASE("datalogger_cfg: encode -> decode round-trip preserves fields",
          "[devices][ap3][datalogger_cfg]") {
    st::devices::ets::DataloggerPreset src;
    src.name = "Cold Start";
    src.description = "fuel trims + ECT for the first 60 s";
    src.log_mon_list = {"ect", "fuel_trim_short", "fuel_trim_long", "rpm",
                        "throttle", "iam"};
    auto const text = st::devices::ets::encode_datalogger_cfg(src);
    auto round = st::devices::ets::decode_datalogger_cfg(text);
