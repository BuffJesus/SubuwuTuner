...
// used in `.ptm` files (commit 97bd2dd, src/devices/ets/src/aes256_ctr.cpp).
//
...
TEST_CASE("ap3 cipher: AES-256-CTR survives a 16 KB chunk boundary",
    ...
    auto const encrypted = st::devices::ets::cipher::aes256_ctr_encrypt(plaintext, kNonce);
    REQUIRE(encrypted.has_value());
    REQUIRE(encrypted->size() == plaintext.size());
    auto const decrypted = st::devices::ets::cipher::aes256_ctr_decrypt(*encrypted, kNonce);
    ...
    auto const enc_two_chunks =
        st::devices::ets::cipher::aes256_ctr_encrypt(first_two_chunks, kNonce);
...
TEST_CASE("ap3 cipher: AES-256-CTR is symmetric (encrypt == decrypt at the wire)",
    ...
    auto const e = st::devices::ets::cipher::aes256_ctr_encrypt(plaintext, kNonce);
    REQUIRE(e.has_value());
    auto const d = st::devices::ets::cipher::aes256_ctr_decrypt(plaintext, kNonce);
    REQUIRE(d.has_value());
    REQUIRE(*e == *d);
}
