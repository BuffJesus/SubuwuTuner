...
// dual-mode: when ST_ETS_HAVE_CIPHER is undefined, the same tests
...
[[maybe_unused]] std::vector<std::uint8_t> load_cipher_fixture(char const *name) {
    std::filesystem::path p = std::filesystem::path{ST_FIXTURE_ETS_CIPHER_DIR} / name;
...
TEST_CASE("ap3 cipher: XTEA-CBC encrypt -> decrypt round-trip preserves bytes",
    ...
    auto encrypted = st::devices::ets::cipher::xtea_cbc_encrypt(plaintext, seed);
...
TEST_CASE("ap3 cipher: XTEA-CBC decrypt rejects short input",
    ...
    auto r = st::devices::ets::cipher::xtea_cbc_decrypt(bad);
...
TEST_CASE("ap3 cipher: base64 encodes \"Man\" to \"TWFu\"", "[devices][ap3][cipher]") {
    ...
    auto enc = st::devices::ets::cipher::base64_encode(m);
    REQUIRE(enc == "TWFu");
}
...
TEST_CASE("ap3 cipher: base64 round-trip on assorted lengths", "[devices][ap3][cipher]") {
    for (std::string_view const txt : {std::string_view{"f"}, std::string_view{"fo"},
        ...
        std::span<std::uint8_t const> bytes{reinterpret_cast<std::uint8_t const *>(txt.data()),
                                            txt.size()};
        auto enc = st::devices::ets::cipher::base64_encode(bytes);
        auto dec = st::devices::ets::cipher::base64_decode(enc);
...
TEST_CASE("ap3 cipher: base64_decode tolerates whitespace and padding",
          "[devices][ap3][cipher]") {
    auto dec = st::devices::ets::cipher::base64_decode("TWFu\n\r  ");
    REQUIRE(dec.has_value());
    REQUIRE(dec->size() == 3);
    REQUIRE((*dec)[0] == 'M');
    REQUIRE((*dec)[1] == 'a');
...
TEST_CASE("ap3 cipher: base64_decode rejects invalid characters",
          "[devices][ap3][cipher]") {
    auto dec = st::devices::ets::cipher::base64_decode("TWF!");
...
TEST_CASE("ap3 cipher: extract_enc_data finds the body between the tags",
    ...
    auto body = st::devices::ets::cipher::extract_enc_data(xml);
...
TEST_CASE("ap3 cipher: extract_enc_data returns ParseError on missing tags",
          "[devices][ap3][cipher]") {
    auto a = st::devices::ets::cipher::extract_enc_data("<root></root>");
    REQUIRE_FALSE(a.has_value());
    REQUIRE(a.error().code() == st::ErrorCode::ParseError);

    auto b = st::devices::ets::cipher::extract_enc_data("<root><encData>oops");
...
TEST_CASE("ap3 cipher: decrypt_ptm_outer round-trips a synthetic .ptm envelope",
    ...
    auto encrypted = st::devices::ets::cipher::xtea_cbc_encrypt(pt_span, seed);
    REQUIRE(encrypted.has_value());
    auto recovered = st::devices::ets::cipher::decrypt_ptm_outer(*encrypted);
...
TEST_CASE("ap3 cipher: decrypt_ptm_outer rejects invalid pad_count",
    ...
    auto r = st::devices::ets::cipher::decrypt_ptm_outer(ptm);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::ParseError);
}
...
TEST_CASE("ap3 cipher fixtures: base64 layer 2 envelope + decode against canonical vectors",
    ...
    auto extracted = st::devices::ets::cipher::extract_enc_data(input_xml);
    ...
    auto inner = st::devices::ets::cipher::base64_decode(*extracted);
...
TEST_CASE("ap3 cipher: AES-256-CTR symmetric round-trip with the custom construction",
    ...
    auto encrypted = st::devices::ets::cipher::aes256_ctr_encrypt(plaintext, kNonce);
    REQUIRE(encrypted.has_value());
    REQUIRE(encrypted->size() == plaintext.size());
    auto decrypted = st::devices::ets::cipher::aes256_ctr_decrypt(*encrypted, kNonce);
    REQUIRE(decrypted.has_value());
    REQUIRE(*decrypted == plaintext);
    // Different nonce → different keystream → wrong plaintext.
    auto wrong = st::devices::ets::cipher::aes256_ctr_decrypt(*encrypted, 0x21U);
...
TEST_CASE("ap3 cipher fixtures: bzip2 decompress layer 4 against canonical vectors",
    ...
    auto decompressed = st::devices::ets::cipher::bzip2_decompress(compressed);
...
TEST_CASE("ap3 cipher: bzip2_decompress rejects non-bzip2 input",
    ...
    auto r = st::devices::ets::cipher::bzip2_decompress(garbage);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::ParseError);
}
...
TEST_CASE("ap3 cipher: bzip2_compress round-trips against decompress",
    ...
    auto compressed = st::devices::ets::cipher::bzip2_compress(input);
...
TEST_CASE("ap3 cipher: encrypt_ptm -> decrypt_ptm logical round-trip",
    ...
    auto r = st::devices::ets::cipher::encrypt_ptm(
    ...
    auto ptm_bytes = st::devices::ets::cipher::encrypt_ptm(
...
TEST_CASE("ap3 cipher: PolicyDenied when build flag is off",
    ...
    auto r = st::devices::ets::cipher::decrypt_ptm_outer(dummy);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::PolicyDenied);

    auto e = st::devices::ets::cipher::xtea_cbc_encrypt(dummy, std::uint32_t{0});
    REQUIRE_FALSE(e.has_value());
    REQUIRE(e.error().code() == st::ErrorCode::PolicyDenied);

    auto d = st::devices::ets::cipher::xtea_cbc_decrypt(dummy);
    REQUIRE_FALSE(d.has_value());
    REQUIRE(d.error().code() == st::ErrorCode::PolicyDenied);

    auto a = st::devices::ets::cipher::aes256_ctr_decrypt(dummy, 0x20U);
    REQUIRE_FALSE(a.has_value());
    REQUIRE(a.error().code() == st::ErrorCode::PolicyDenied);

    auto b = st::devices::ets::cipher::bzip2_decompress(dummy);
    REQUIRE_FALSE(b.has_value());
    REQUIRE(b.error().code() == st::ErrorCode::PolicyDenied);

    auto f = st::devices::ets::cipher::decrypt_ptm(dummy);
