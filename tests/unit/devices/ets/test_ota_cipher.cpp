...
TEST_CASE("parse_ota_img extracts header / body / trailer",
    ...
    auto r = st::devices::ets::cipher::parse_ota_img(img);
    REQUIRE(r.has_value());
...
TEST_CASE("parse_ota_img refuses input shorter than the minimum envelope",
    ...
    auto r = st::devices::ets::cipher::parse_ota_img(too_short);
...
TEST_CASE("parse_ota_img / build_ota_img round-trip preserves bytes",
    ...
    // every field matches.
    st::devices::ets::cipher::OtaImgWrapper src;
    ...
    auto const bytes = st::devices::ets::cipher::build_ota_img(src);
    auto round = st::devices::ets::cipher::parse_ota_img(bytes);
...
TEST_CASE("blowfish_ctr_double_decrypt gated OFF returns PolicyDenied",
    ...
    auto const r = st::devices::ets::cipher::blowfish_ctr_double_decrypt(ct, 0x42);
    REQUIRE_FALSE(r.has_value());
...
TEST_CASE("decrypt_ota_img gated OFF returns PolicyDenied",
    ...
    img[2] = 0xBE;
    img[3] = 0xEF;
    auto const r = st::devices::ets::cipher::decrypt_ota_img(img);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::PolicyDenied);
#endif
}
...
std::vector<std::uint8_t> load_blowfish_fixture(std::string_view name) {
    std::filesystem::path const path =
        std::filesystem::path{ST_FIXTURE_ETS_CIPHER_DIR} / name;
...
TEST_CASE("blowfish_ctr_double_decrypt fixture-pinned byte-identity",
    ...
    auto encrypted = st::devices::ets::cipher::blowfish_ctr_double_encrypt(
        plaintext, kBaseCtr);
    ...
    auto decrypted = st::devices::ets::cipher::blowfish_ctr_double_decrypt(
        ciphertext, kBaseCtr);
...
TEST_CASE("decrypt_ota_img end-to-end against full .img fixture",
    ...
    auto decrypted = st::devices::ets::cipher::decrypt_ota_img(img);
    REQUIRE(decrypted.has_value());
    REQUIRE(decrypted->size() == plaintext.size());
    for (std::size_t i = 0; i < plaintext.size(); ++i) {
        REQUIRE((*decrypted)[i] == plaintext[i]);
