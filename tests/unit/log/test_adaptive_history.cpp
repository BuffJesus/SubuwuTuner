...
TEST_CASE("snapshot_from_samples returns empty on empty input",
          "[adaptive][snapshot]") {
    auto const s = snapshot_from_samples({}, 4, basic_mapping(),
                                          daily_unix_cfg());
...
TEST_CASE("snapshot_from_samples buckets by day at default 86400 s",
    ...
    auto const s = snapshot_from_samples(samples, 4, basic_mapping(),
                                          daily_unix_cfg());
...
TEST_CASE("snapshot_from_samples computes drift_per_second slope",
          "[adaptive][snapshot][drift]") {
    ...
    REQUIRE_THAT(s.series[0].drift_per_second,
                  WithinAbs(10.0 / 86400.0, 1e-9));
}

TEST_CASE("snapshot_from_samples respects min_samples_per_bucket",
          "[adaptive][snapshot][filter]") {
    auto const samples = flatten({
...
TEST_CASE("snapshot_from_samples normalizes UnixMillis timestamps",
          "[adaptive][snapshot][units]") {
...
TEST_CASE("snapshot_from_samples skips signals not in mapping",
          "[adaptive][snapshot][missing]") {
...
TEST_CASE("snapshot_from_samples records time-span fields",
          "[adaptive][snapshot][span]") {
    auto const samples = flatten({
        { 100.0,   1.0, 1.0, 0.0},
        { 1100.0,  2.0, 1.0, 0.0},
        { 5000.0,  3.0, 1.0, 0.0},
...
TEST_CASE("snapshot_from_csv rejects mapping with no signal columns",
          "[adaptive][csv][validate]") {
    ColumnMapping m  = basic_mapping();
    for (std::size_t i = 0; i < kSignalCount; ++i) m.signal_idx[i] = kNoColumn;
    auto const   r   = snapshot_from_csv("/tmp/whatever.csv", m,
                                          daily_unix_cfg());
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::InvalidArgument);
...
TEST_CASE("snapshot_from_csv reads a small log",
          "[adaptive][csv][roundtrip]") {
    auto const path = std::filesystem::temp_directory_path()
                      / "subuwutuner_test_adaptive.csv";
    {
