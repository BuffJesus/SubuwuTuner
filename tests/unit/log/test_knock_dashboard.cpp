...
TEST_CASE("snapshot_from_samples gates out samples below min_rpm",
          "[knock][snapshot][gate]") {
    PidMapping const   m       = h4_mapping();
    WindowConfig const c       = default_cfg();
...
TEST_CASE("snapshot_from_samples computes delta from all-cyl mean",
          "[knock][snapshot][delta]") {
    PidMapping const   m       = h4_mapping();
    WindowConfig const c       = default_cfg();
    auto const         samples = flatten({
        // rpm,  load,  flkc1, flkc2, flkc3, flkc4
...
TEST_CASE("snapshot_from_samples supports H6 cylinder count",
          "[knock][snapshot][h6]") {
    PidMapping m{};
    m.cylinder_count   = 6;
    m.rpm_idx          = 0;
