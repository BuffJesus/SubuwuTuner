...
std::vector<double> synth_clean_tip_in(double peak_overshoot = 1.5) {
    ...
    // Idle / cruise lead-in
    for (double t = 0.0; t < 1.0; t += 0.1) add(t, 0.0, 0.0, 20.0);
    // The step
...
TEST_CASE("snapshot_from_samples detects a clean tip-in",
          "[ebcs][snapshot][detect]") {
...
TEST_CASE("snapshot_from_samples computes overshoot percentage",
          "[ebcs][snapshot][overshoot]") {
...
TEST_CASE("snapshot_from_samples flags large overshoot as Spiked",
          "[ebcs][snapshot][spiked]") {
...
TEST_CASE("snapshot_from_samples returns no events when no tip-in",
          "[ebcs][snapshot][nodetect]") {
    // Steady cruise — no throttle step.
    std::vector<double> samples;
    auto add = [&](double t) {
        samples.insert(samples.end(),
                       {t, 5.0, 5.0, 30.0, 25.0, 2500.0});
    };
    for (double t = 0.0; t < 5.0; t += 0.1) add(t);
    auto const s = snapshot_from_samples(samples, 6, basic_mapping(),
                                         default_cfg());
    REQUIRE(s.total_event_count == 0);
...
TEST_CASE("snapshot_from_samples surfaces overshoot suggestion",
          "[ebcs][snapshot][suggest]") {
