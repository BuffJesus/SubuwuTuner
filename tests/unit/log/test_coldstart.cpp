...
Methodology default_methodology() {
    ...
    m.target_lambda_vs_ect.points = {
        {-20.0, 0.80}, {0.0, 0.85}, {20.0, 0.90}, {40.0, 0.95}, {55.0, 1.00},
    };
    return m;
}
...
TEST_CASE("TargetLambdaCurve::sample returns 1.0 on empty curve", "[coldstart][curve]") {
    TargetLambdaCurve c;
...
TEST_CASE("TargetLambdaCurve::sample clamps below first / above last point", "[coldstart][curve]") {
    TargetLambdaCurve c;
...
TEST_CASE("TargetLambdaCurve::sample linearly interpolates between points", "[coldstart][curve]") {
    TargetLambdaCurve c;
...
TEST_CASE("snapshot_from_samples classifies samples by RPM + ECT", "[coldstart][snapshot][phase]") {
    ...
    auto const s =
        snapshot_from_samples(samples, 6, basic_mapping(), default_cfg(), default_methodology());
...
TEST_CASE("snapshot_from_samples bins cold-regime samples by ECT", "[coldstart][snapshot][bin]") {
...
TEST_CASE("snapshot_from_samples excludes warm samples from bins", "[coldstart][snapshot][bin]") {
    auto const samples = flatten({
...
TEST_CASE("snapshot_from_samples returns empty on empty input", "[coldstart][snapshot]") {
    auto const s =
        snapshot_from_samples({}, 6, basic_mapping(), default_cfg(), default_methodology());
...
TEST_CASE("snapshot_from_csv reports FileNotFound on missing path", "[coldstart][csv]") {
...
TEST_CASE("snapshot_from_csv rejects mapping without mandatory columns",
    ...
    m.ect_idx = kNoColumn;
    auto const r = snapshot_from_csv("/tmp/whatever.csv", m, default_cfg(), default_methodology());
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::InvalidArgument);
}

TEST_CASE("snapshot_from_csv reads a small log", "[coldstart][csv][roundtrip]") {
    auto const path = std::filesystem::temp_directory_path() / "subuwutuner_test_coldstart.csv";
    {
    ...
    auto const r =
        snapshot_from_csv(path.string(), basic_mapping(), default_cfg(), default_methodology());
    REQUIRE(r.has_value());
