...
can_ns::Frame make_frame(std::int64_t ts_ns, can_ns::BusId bus, std::uint32_t id, bool extended,
...
TEST_CASE("parse_asc handles a minimal single-frame log", "[can][asc][parse]") {
    ...
    REQUIRE(f.dlc == 8);
    std::vector<std::uint8_t> const want{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77};
    for (std::uint8_t i = 0; i < 8; ++i) {
        REQUIRE(f.data[i] == want[i]);
    }
}

TEST_CASE("parse_asc preserves order across multiple frames + buses", "[can][asc][parse]") {
    constexpr std::string_view text = "Begin Triggerblock\n"
...
TEST_CASE("parse_asc recognises extended (29-bit) IDs", "[can][asc][parse]") {
    constexpr std::string_view text = "   0.000100 1  12345678x       Rx   d 1 FF\n";
    auto const r = can_ns::parse_asc(text);
    REQUIRE(r.has_value());
    REQUIRE(r->size() == 1);
...
TEST_CASE("parse_asc skips header / comment lines silently", "[can][asc][parse]") {
    // Header noise the parser must tolerate.
    constexpr std::string_view text = "date Wed Jan 01 00:00:00 1970\n"
                                      "base hex  timestamps absolute\n"
                                      "internal events logged\n"
                                      "// version 13.0.0\n"
...
TEST_CASE("parse_asc accepts zero-DLC (heartbeat) frames", "[can][asc][parse]") {
    constexpr std::string_view text = "   0.000000 1  100             Rx   d 0\n";
    auto const r = can_ns::parse_asc(text);
    REQUIRE(r.has_value());
    REQUIRE(r->size() == 1);
...
TEST_CASE("parse_asc rejects a frame line with a DLC > 8", "[can][asc][parse][error]") {
    constexpr std::string_view text =
...
TEST_CASE("parse_asc handles CRLF line endings", "[can][asc][parse]") {
    std::string text;
...
TEST_CASE("format_asc emits parseable canonical output", "[can][asc][format]") {
...
TEST_CASE("read_asc and write_asc round-trip via the filesystem", "[can][asc][file]") {
    auto const tmp = std::filesystem::temp_directory_path() /
                     ("st_can_test_" + std::to_string(std::random_device{}()) + ".asc");

    std::vector<can_ns::Frame> const frames{
        make_frame(0, can_ns::BusId::Hs, 0x140, false, {0x11, 0x22, 0x33, 0x44}),
        make_frame(500'000, can_ns::BusId::Hs, 0x140, false,
                   {0x11, 0x22, 0x33, 0x45}), // byte 3 flipped (1 bit)
        make_frame(1'000'000, can_ns::BusId::Ms, 0x300, false, {0x00}),
    };
...
TEST_CASE("read_asc errors clearly when the file does not exist", "[can][asc][file][error]") {
