...
TEST_CASE("RamAllocator: empty region refuses every claim", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("RamAllocator: zero-size claim is invalid", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("RamAllocator: sequential claims bump the cursor", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("RamAllocator: claims round up to alignment", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("RamAllocator: refuses on region exhaustion", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("RamAllocator: reset moves the cursor back to base", "[feature_codegen][ram_allocator]") {
    cg::RamAllocator a{0x40000000, 16};
...
TEST_CASE("RamAllocator: alignment-1 means no alignment", "[feature_codegen][ram_allocator]") {
...
TEST_CASE("select_backend(VA) returns an SH-2A backend", "[feature_codegen][select]") {
    auto b = cg::select_backend("VA");
    REQUIRE(b.has_value());
    REQUIRE((*b)->arch() == cg::Arch::Sh2a);
}

TEST_CASE("select_backend(VB) returns an RH850 backend", "[feature_codegen][select]") {
    auto b = cg::select_backend("VB");
    REQUIRE(b.has_value());
    REQUIRE((*b)->arch() == cg::Arch::Rh850);
}

TEST_CASE("select_backend is case-insensitive on the platform name", "[feature_codegen][select]") {
...
TEST_CASE("select_backend refuses an empty platform string", "[feature_codegen][select]") {
...
TEST_CASE("Sh2aBackend reports its arch", "[feature_codegen][sh2a]") {
...
TEST_CASE("patch_from_toml: empty text returns nullopt", "[feature_codegen][patch_toml]") {
...
TEST_CASE("patch_to_toml: single-hook patch round-trips", "[feature_codegen][patch_toml]") {
    cg::PatchObject p;
    p.arch = cg::Arch::Sh2a;
    cg::HookPatch h;
...
TEST_CASE("patch_to_toml: multi-hook patch keeps hook order", "[feature_codegen][patch_toml]") {
    cg::PatchObject p;
    p.arch = cg::Arch::Rh850;
    cg::HookPatch h1;
    h1.symbol = "hook_a";
    h1.splice_address = 0x100;
    h1.code = {0x01, 0x02};
    cg::HookPatch h2;
    h2.symbol = "hook_b";
    h2.splice_address = 0x200;
    h2.code = {0x03, 0x04};
...
TEST_CASE("patch_from_toml: malformed TOML returns ParseError", "[feature_codegen][patch_toml]") {
    auto r = cg::patch_from_toml("[patch\nbroken");
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::ParseError);
}

TEST_CASE("patch_from_toml: missing arch returns ParseError", "[feature_codegen][patch_toml]") {
    auto r = cg::patch_from_toml(R"toml(
[patch]
)toml");
    REQUIRE_FALSE(r.has_value());
...
TEST_CASE("patch_from_toml: unknown arch returns ParseError", "[feature_codegen][patch_toml]") {
    auto r = cg::patch_from_toml(R"toml(
[patch]
arch = "v850"
)toml");
...
TEST_CASE("patch_from_toml: accepts lowercase hex too", "[feature_codegen][patch_toml]") {
    auto r = cg::patch_from_toml(R"toml(
[patch]
arch = "sh2a"
[[patch.hook]]
...
code = "deadbeef"
    ...
    REQUIRE(r.has_value());
    REQUIRE(r->has_value());
    REQUIRE((*r)->hooks[0].code == std::vector<std::uint8_t>{0xDE, 0xAD, 0xBE, 0xEF});
}
