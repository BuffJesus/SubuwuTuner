...
ir::Instruction load_hook_input(ir::ValueId result_id, std::string hook_id, std::string pin_name,
                                st::feature::PinType type = st::feature::PinType::Int) {
    ir::Instruction ins{};
...
ir::Instruction call_primitive(ir::ValueId result_id, std::string symbol,
...
ir::Instruction store(std::string symbol, std::string pin_name, ir::ValueId operand) {
    ir::Instruction ins{};
...
[[nodiscard]] std::uint16_t be16_at(std::vector<std::uint8_t> const &code, std::size_t offset) {
    return static_cast<std::uint16_t>((static_cast<std::uint16_t>(code.at(offset)) << 8U) |
                                      static_cast<std::uint16_t>(code.at(offset + 1)));
}

[[nodiscard]] std::uint32_t be32_at(std::vector<std::uint8_t> const &code, std::size_t offset) {
...
TEST_CASE("Sh2aBackend: empty module compiles to an empty PatchObject", "[feature_codegen][sh2a]") {
...
TEST_CASE("Sh2aBackend: LoadConstant with no consumer is dead code", "[feature_codegen][sh2a]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: LoadConstant + Store emits 20-byte sequence", "[feature_codegen][sh2a]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    ir::Module m;
    m.instructions.push_back(load_const_int(1, 42));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 1));
...
TEST_CASE("Sh2aBackend: two stores to the same pin share a RAM slot", "[feature_codegen][sh2a]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: distinct pins get distinct RAM slots", "[feature_codegen][sh2a]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    m.instructions.push_back(load_const_int(2, 22));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 1));
    m.instructions.push_back(store("after_fuel_calc", "second_override", 2));
    auto r = backend.compile(m, *def);
    REQUIRE(r.has_value());
    REQUIRE(r->hooks.size() == 1);
    auto const &hp = r->hooks[0];
...
TEST_CASE("Sh2aBackend: missing hook id returns InvalidArgument", "[feature_codegen][sh2a]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    ir::Module m;
    m.instructions.push_back(load_const_int(1, 42));
    m.instructions.push_back(store("ghost_hook", "commanded_pw_override", 1));
    auto r = backend.compile(m, *def);
...
TEST_CASE("Sh2aBackend: hook without free_ram returns InvalidArgument", "[feature_codegen][sh2a]") {
...
TEST_CASE("Sh2aBackend: LoadHookInput Float emits same MOV.L shape",
    ...
    m.instructions.push_back(
        load_hook_input(1, "after_fuel_calc", "rpm", st::feature::PinType::Float));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 1));
    auto r = backend.compile(m, *def);
    REQUIRE(r.has_value());
    REQUIRE(r->hooks.size() == 1);
...
TEST_CASE("Sh2aBackend: LoadHookInput referencing unknown pin fails",
    ...
    cg::Sh2aBackend backend;
    ir::Module m;
    m.instructions.push_back(load_hook_input(1, "after_fuel_calc", "ghost_pin"));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 1));
    auto r = backend.compile(m, *def);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::InvalidArgument);
...
TEST_CASE("Sh2aBackend: LoadHookInput pin without address fails",
    ...
    cg::Sh2aBackend backend;
    ir::Module m;
    m.instructions.push_back(load_hook_input(1, "after_fuel_calc", "no_addr_pin"));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 1));
    auto r = backend.compile(m, *def);
    REQUIRE_FALSE(r.has_value());
    REQUIRE(r.error().code() == st::ErrorCode::InvalidArgument);
...
TEST_CASE("Sh2aBackend: Float LoadConstant special values", "[feature_codegen][sh2a][float]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: Bool LoadConstant canonical 0/1 widening",
    ...
    auto const cases = std::vector<Case>{
        {0.0, 0U},  {1.0, 1U}, {0.4, 0U}, // below threshold
...
TEST_CASE("Sh2aBackend: nested add(add(LC,LC), LC) → Store allocates a slot",
    ...
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 5));
...
TEST_CASE("Sh2aBackend: 3-level nested add tree → Store", "[feature_codegen][sh2a][nested]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 7));
...
TEST_CASE("Sh2aBackend: nested add with LoadHookInput leaf", "[feature_codegen][sh2a][nested]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: subtract_int(LC, LC) → Store emits SUB", "[feature_codegen][sh2a][sub]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: multiply_int with HookInput operand", "[feature_codegen][sh2a][mul]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: compare_eq emits CMP/EQ + MOVT", "[feature_codegen][sh2a][compare]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: compare result_type must be Bool", "[feature_codegen][sh2a][compare]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: and_bool(LC, LC) → Store emits AND", "[feature_codegen][sh2a][bool_prim]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: or_bool(LC, LC) → Store emits OR", "[feature_codegen][sh2a][bool_prim]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: and_bool rejects Int operand", "[feature_codegen][sh2a][bool_prim]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: not_bool with wrong operand count", "[feature_codegen][sh2a][bool_prim]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: select_int(LC, LC, LC) → Store branch layout",
    ...
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 4));
...
TEST_CASE("Sh2aBackend: select_int with HookInput cond", "[feature_codegen][sh2a][select]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    m.instructions.push_back(
        load_hook_input(1, "after_fuel_calc", "rpm", st::feature::PinType::Bool));
    m.instructions.push_back(load_const_int(2, 100));
...
TEST_CASE("Sh2aBackend: select_int nested inside add_int",
    ...
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 6));
...
TEST_CASE("Sh2aBackend: select_int with wrong arity", "[feature_codegen][sh2a][select]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
...
TEST_CASE("Sh2aBackend: add_float emits FADD with FPUL transfers",
    ...
    m.instructions.push_back(call_primitive(3, "add_float", {1, 2}, st::feature::PinType::Float));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 3));
    ...
    for (std::size_t i = 0; i + 1 < hp.code.size(); i += 2) {
        std::uint16_t inst = be16_at(hp.code, i);
        if (inst == 0x405A)
            seen_lds_r0_fpul = true;
        if (inst == 0xF00D)
            seen_fsts_fpul_fr0 = true;
        if (inst == 0xF10D)
            seen_fsts_fpul_fr1 = true;
        if (inst == 0xF100)
            seen_fadd = true;
        if (inst == 0xF11D)
            seen_flds_fr1_fpul = true;
        if (inst == 0x015A)
            seen_sts_fpul_r1 = true;
    }
...
TEST_CASE("Sh2aBackend: subtract_float emits FSUB with op1 in FR1",
    ...
    m.instructions.push_back(load_const_float(2, 2.0));
    m.instructions.push_back(
        call_primitive(3, "subtract_float", {1, 2}, st::feature::PinType::Float));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 3));
    ...
    for (std::size_t i = 0; i + 1 < r->hooks[0].code.size(); i += 2) {
        if (be16_at(r->hooks[0].code, i) == 0xF101)
            seen_fsub = true;
    }
    REQUIRE(seen_fsub);
}

TEST_CASE("Sh2aBackend: multiply_float emits FMUL", "[feature_codegen][sh2a][float]") {
    auto def = st::Definition::from_toml_string(kPackFloatHookToml);
    REQUIRE(def.has_value());
    ...
    m.instructions.push_back(load_const_float(2, 2.0));
    m.instructions.push_back(
        call_primitive(3, "multiply_float", {1, 2}, st::feature::PinType::Float));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 3));
    ...
    for (std::size_t i = 0; i + 1 < r->hooks[0].code.size(); i += 2) {
        if (be16_at(r->hooks[0].code, i) == 0xF102)
            seen_fmul = true;
    }
    REQUIRE(seen_fmul);
}
...
TEST_CASE("Sh2aBackend: divide_float emits FDIV with op1 in FR1",
    ...
    m.instructions.push_back(
        call_primitive(3, "divide_float", {1, 2}, st::feature::PinType::Float));
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 3));
    ...
    bool seen_fdiv = false;
    for (std::size_t i = 0; i + 1 < r->hooks[0].code.size(); i += 2) {
        if (be16_at(r->hooks[0].code, i) == 0xF103)
            seen_fdiv = true;
    }
    REQUIRE(seen_fdiv);
}
...
TEST_CASE("Sh2aBackend: compare_eq_float emits FCMP/EQ", "[feature_codegen][sh2a][float]") {
    auto def = st::Definition::from_toml_string(kPackOneHookToml);
    REQUIRE(def.has_value());
    ...
    for (std::size_t i = 0; i + 1 < r->hooks[0].code.size(); i += 2) {
        if (be16_at(r->hooks[0].code, i) == 0xF104)
            seen_fcmp_eq = true;
    }
...
TEST_CASE("Sh2aBackend: cross-hook LoadHookInput → Store compiles",
    ...
    cg::Sh2aBackend backend;
    ir::Module m;
    m.instructions.push_back(load_hook_input(1, "read_rpm", "rpm", st::feature::PinType::Int));
...
TEST_CASE("Sh2aBackend: fanned-out CallPrimitive result is emitted once",
    ...
    m.instructions.push_back(store("after_fuel_calc", "commanded_pw_override", 8));
