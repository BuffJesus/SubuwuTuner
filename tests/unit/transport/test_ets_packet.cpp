...
std::vector<std::uint8_t> load_ap3_fixture(char const *name) {
    std::filesystem::path p = std::filesystem::path{ST_FIXTURE_ETS_DIR} / name;
...
TEST_CASE("ap3 packet: decode_header reads sec12.4 PutFile header -- u24-BE not u16-LE",
    ...
    auto header = st::transport::ets::decode_header(kPutFileHeader);
    REQUIRE(header.has_value());
...
TEST_CASE("ap3 packet: sec12.5 setup ACK round-trips through verify_crc and decode_header",
          "[transport][ap3]") {
    auto header = st::transport::ets::decode_header(kSetupAckPacket);
    REQUIRE(header.has_value());
    REQUIRE(header->type == 0x01); // ResponseType::Info
    REQUIRE(header->wire_len == 5U);
    REQUIRE(header->body_size == 1U);
    auto status = st::transport::ets::verify_crc(kSetupAckPacket);
    REQUIRE(status.has_value());
}
...
TEST_CASE("ap3 packet: encode_packet round-trips against sec12.2 UserInfo vector",
    ...
    auto encoded = st::transport::ets::encode_packet(0x28, kBody);
...
TEST_CASE("ap3 packet: decode_packet_body rejects a mutated CRC", "[transport][ap3]") {
    ...
    auto body = st::transport::ets::decode_packet_body(mutated);
...
TEST_CASE("ap3 packet: decode_header rejects a bad sync magic", "[transport][ap3]") {
    ...
    auto header = st::transport::ets::decode_header(mutated);
...
TEST_CASE("ap3 fixtures: packet_cmd28_userinfo.bin matches the inline sec12.2 vector",
    ...
    auto status = st::transport::ets::verify_crc(fixture);
    REQUIRE(status.has_value());
}
...
TEST_CASE("ap3 fixtures: packet_cmd22_setup_ack.bin decodes to type=Info wire_len=5",
    ...
    auto header = st::transport::ets::decode_header(fixture);
    REQUIRE(header.has_value());
    REQUIRE(header->type == 0x01);
    REQUIRE(header->wire_len == 5U);
    REQUIRE(st::transport::ets::verify_crc(fixture).has_value());
}
...
TEST_CASE("ap3 sec6.0: is_blocked_command flags every spec-listed cmd byte",
    ...
    for (auto b : kBlocked) {
        ...
        REQUIRE(st::transport::ets::is_blocked_command(b));
    }
    // Above-range bytes (> 0x31) are all blocked.
    REQUIRE(st::transport::ets::is_blocked_command(0x32));
    REQUIRE(st::transport::ets::is_blocked_command(0x40));
    REQUIRE(st::transport::ets::is_blocked_command(0xFF));
}
...
TEST_CASE("ap3 sec6.0: is_blocked_command does NOT flag the safe file-vault set",
    ...
    for (auto s : kSafe) {
        ...
        REQUIRE_FALSE(st::transport::ets::is_blocked_command(s));
    }
    REQUIRE_FALSE(st::transport::ets::is_blocked_command(0x26)); // ListFiles
    REQUIRE_FALSE(st::transport::ets::is_blocked_command(0x28)); // UserInfo
    REQUIRE_FALSE(st::transport::ets::is_blocked_command(0x2E)); // HardwareType
    REQUIRE_FALSE(st::transport::ets::is_blocked_command(0x30)); // VehicleManufacturer
    REQUIRE_FALSE(st::transport::ets::is_blocked_command(0x31)); // ApManufacturer
}
...
TEST_CASE("ap3 sec6.0: encode_packet returns PolicyDenied for blocked cmd bytes",
    ...
    for (auto b : kBlocked) {
        INFO("blocked cmd byte 0x" << std::hex << static_cast<int>(b));
        auto encoded = st::transport::ets::encode_packet(b, std::span<std::uint8_t const>{});
...
TEST_CASE("ap3 packet: encode_packet rejects oversized bodies", "[transport][ap3]") {
    ...
    auto enc = st::transport::ets::encode_packet(0x23, small);
    REQUIRE(enc.has_value());
    REQUIRE(enc->size() == st::transport::ets::kHeaderSize + small.size() +
