...
std::filesystem::path write_temp_trace(std::string_view content) {
    auto const path = std::filesystem::temp_directory_path() /
                      ("test_uds_trace_" +
                       std::to_string(static_cast<unsigned>(std::rand())) +
                       ".uds");
...
TEST_CASE("parse_uds_trace: canonical alternating exchange",
          "[transport][uds_trace]") {
    auto const path = write_temp_trace(
        "> 22 F1 90\n"
        "< 62 F1 90 01 02 03\n"
        "> 22 F1 80\n"
        "< 62 F1 80 04 05 06\n");
    ...
    REQUIRE(pairs[0].response ==
            std::vector<std::uint8_t>{0x62, 0xF1, 0x90, 0x01, 0x02, 0x03});
...
TEST_CASE("parse_uds_trace: missing file error", "[transport][uds_trace]") {
    std::vector<st::transport::UdsTracePair> pairs;
    std::string err;
    REQUIRE_FALSE(st::transport::parse_uds_trace(
        "/nonexistent/path/should/not/exist.uds", pairs, err));
    REQUIRE(err.find("cannot open") != std::string::npos);
}

TEST_CASE("parse_uds_trace: empty trace file rejected",
          "[transport][uds_trace]") {
...
TEST_CASE("parse_uds_trace: unmatched request rejected",
          "[transport][uds_trace]") {
    auto const path = write_temp_trace(
        "> 22 F1 90\n"
        "< 62 F1 90\n"
...
TEST_CASE("parse_uds_trace: two requests in a row rejected",
          "[transport][uds_trace]") {
    auto const path = write_temp_trace(
        "> 22 F1 90\n"
        "> 22 F1 80\n");
...
TEST_CASE("parse_uds_trace: bad hex byte rejected",
          "[transport][uds_trace]") {
    auto const path = write_temp_trace(
        "> 22 ZZ 90\n"
        "< 62 F1 90\n");
