...
TEST_CASE("native::crc16_ccitt_false: empty input → 0xFFFF (init)",
          "[transport][native]") {
    std::vector<std::uint8_t> empty;
    REQUIRE(nat::crc16_ccitt_false(std::span<std::uint8_t const>{empty})
            == 0xFFFFU);
}
...
TEST_CASE("native::crc16_ccitt_false: '123456789' → 0x29B1 (RevEng vector)",
    ...
    std::span<std::uint8_t const> bytes{
        reinterpret_cast<std::uint8_t const *>(ascii.data()),
        ascii.size()};
    REQUIRE(nat::crc16_ccitt_false(bytes) == 0x29B1U);
}

TEST_CASE("native::crc16_ccitt_false: single 0x00 byte → 0xE1F0",
          "[transport][native]") {
...
TEST_CASE("native::encode_request: zero-payload Hello frame, fixed seq 0",
          "[transport][native]") {
...
TEST_CASE("native::encode_request: max-size payload accepted (~64 KiB)",
          "[transport][native]") {
...
namespace {
std::vector<std::uint8_t> build_response_frame(
    std::uint8_t seq, std::uint8_t response_op,
...
TEST_CASE("native::decode_frame: too short (< 7 B) → ParseError",
          "[transport][native]") {
...
TEST_CASE("native::decode_frame: wrong SOF byte → ParseError",
          "[transport][native]") {
...
TEST_CASE("native::decode_frame: bad CRC → ParseError",
          "[transport][native]") {
...
TEST_CASE("native::decode_frame: trailing bytes → ParseError",
          "[transport][native]") {
    auto bytes = build_response_frame(0, 0x81U, {});
...
TEST_CASE("native::decode_frame: error frame parses out seq + request_op + err",
    ...
    auto bytes = build_response_frame(
        0,  // outer seq irrelevant for error frames
...
TEST_CASE("native::decode_frame: event frame splits event_code + payload",
          "[transport][native]") {
...
TEST_CASE("native::decode_frame: empty event payload → ParseError",
          "[transport][native]") {
...
TEST_CASE("native::encode_request round-trips THROUGH decode_frame for "
    ...
    constexpr nat::Opcode all[] = {
        nat::Opcode::Hello,        nat::Opcode::Connect,
        nat::Opcode::Disconnect,   nat::Opcode::SendBytes,
        nat::Opcode::RecvBytes,    nat::Opcode::SetConfig,
        nat::Opcode::StreamStart,  nat::Opcode::StreamStop,
        nat::Opcode::ReadBatteryVoltage,
        nat::Opcode::Reset,
    };
    for (auto op : all) {
        ...
        (*enc)[2] = static_cast<std::uint8_t>(
            (*enc)[2] | nat::kResponseMask);
        auto const new_crc = nat::crc16_ccitt_false(
            std::span{enc->data(), enc->size() - 2});
