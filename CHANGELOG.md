...
### Added
- **Custom Feature Designer compile preview** — lowers the active graph through `st::feature_codegen`, selects the backend from the canonical dotted platform id, and reports architecture, IR, hook, code-byte, and RAM-byte totals without mutating the project ROM. Invalid addresses are rejected before code generation, and zero-hook output is called out explicitly.
- **Autonomous Designer GUI smoke coverage** — exercises node placement, wiring, defaults, context menus, compilation, dirty-document confirmation, and discard behavior across a 13-frame visual scenario.
- **Save-time project integrity verification** — persisted projects are reopened and checked against ROM bytes, canonical edit histories, metadata, additional ROMs, and checkpoint warnings before the GUI reports a verified save.
