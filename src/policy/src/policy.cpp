...
std::string_view profile_name(Profile p) noexcept {
    switch (p) {
        case Profile::MotorsportOnly: return "motorsport-only";
        case Profile::AlbertaCa:      return "alberta-ca";
        case Profile::EuRoadworthy:   return "eu-roadworthy";
        case Profile::CaliforniaUs:   return "california-us";
    }
    return "motorsport-only";
}

std::optional<Profile> parse_profile(std::string_view s) noexcept {
    if (s == "motorsport-only") return Profile::MotorsportOnly;
    if (s == "alberta-ca")      return Profile::AlbertaCa;
    if (s == "eu-roadworthy")   return Profile::EuRoadworthy;
    if (s == "california-us")   return Profile::CaliforniaUs;
    return std::nullopt;
}
