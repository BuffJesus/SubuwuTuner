...
Signal const *Database::find_signal(std::uint32_t    msg_id,
...
bool parse_message_header(std::string_view line, Message &msg,
                          std::string &err) {
    std::size_t pos = 0;
    auto const  tag = next_token(line, pos);
    if (tag != "BO_") return false;
...
bool parse_signal_row(std::string_view line, Signal &sig, std::string &err) {
    ...
    auto const bits_tok = next_token(line, pos);
    {
        ...
        if (pipe == std::string_view::npos || at == std::string_view::npos
            || pipe >= at) {
        ...
        if (!parse_size(start_sv, sig.start_bit)
            || !parse_size(len_sv, sig.length_bits)) {
        ...
        if (trimmed.empty()) {
            ...
            if (!parse_message_header(trimmed, m, err)) {
                return failure(ErrorCode::ParseError,
                               "dbc: line " + std::to_string(line_no)
                                   + ": " + err);
            }
            current      = std::move(m);
            have_current = true;
...
void emit_signal(std::string &out, Signal const &s) {
    ...
    std::snprintf(num_buf, sizeof(num_buf), "%zu|%zu@",
                  s.start_bit, s.length_bits);
    out += num_buf;
...
void emit_message(std::string &out, Message const &m) {
    char num_buf[32];
    auto const wire_id =
        m.id | (m.extended ? kExtendedFlag : 0U);
    std::snprintf(num_buf, sizeof(num_buf), "BO_ %u ",
...
Status write_dbc(std::filesystem::path const &path, Database const &db) {
    ...
    if (!out) {
        return failure(ErrorCode::IoFailure,
                       "dbc: cannot open for write: " + path.string());
    }
    auto const text = format_dbc(db);
    out.write(text.data(), static_cast<std::streamsize>(text.size()));
    if (!out) {
        return failure(ErrorCode::IoFailure,
                       "dbc: write failed: " + path.string());
    }
    return ok();
}
...
std::uint64_t extract_raw_bits(std::span<std::uint8_t const> data,
                                Signal const                 &sig) noexcept {
    if (sig.length_bits == 0 || sig.length_bits > 64) return 0;
    ...
    for (std::size_t i = 0; i < sig.length_bits; ++i) {
        ...
        if (bit_index >= total_bits) return 0;
        std::size_t const byte_index   = bit_index / 8;
        std::size_t const bit_in_byte  = bit_index % 8;
        std::uint64_t const bit_value =
            (data[byte_index] >> bit_in_byte) & 0x1U;
...
double decode_signal(std::span<std::uint8_t const> data,
                     Signal const                 &sig) noexcept {
    std::uint64_t const raw = extract_raw_bits(data, sig);
    double              value;
    if (sig.sign == SignKind::Signed && sig.length_bits > 0
        && sig.length_bits < 64) {
        ...
        if ((raw & sign_bit) != 0) {
            ...
            std::uint64_t const mask =
                ~((sig.length_bits == 64)
                      ? 0ULL
                      : ((1ULL << sig.length_bits) - 1));
            auto const signed_raw =
