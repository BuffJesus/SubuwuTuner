...
[[nodiscard]] Status write_dbc(std::filesystem::path const &path,
                                Database const &db);
...
[[nodiscard]] std::uint64_t extract_raw_bits(
    std::span<std::uint8_t const> data, Signal const &sig) noexcept;

[[nodiscard]] double decode_signal(
    std::span<std::uint8_t const> data, Signal const &sig) noexcept;
