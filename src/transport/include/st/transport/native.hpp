...
[[nodiscard]] std::uint16_t crc16_ccitt_false(std::span<std::uint8_t const> bytes) noexcept;
...
encode_request(std::uint8_t seq, Opcode op, std::span<std::uint8_t const> payload);
...
//   - buffer is shorter than the minimum header (SOF + seq + op + 2-byte len + 2-byte CRC = 7
//   bytes),
