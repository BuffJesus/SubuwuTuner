    ...
    switch (cfg.kind) {
        ...
        case LinkKind::CanFd:
            return failure(ErrorCode::InvalidArgument,
                           "j2534::connect_params_for: CAN-FD is not "
                           "in J2534 v04.04; needs a v05.00-capable "
                           "adapter, which we don't target yet");
    }
    if (cfg.baud <= 0) {
        return failure(ErrorCode::InvalidArgument,
                       "j2534::connect_params_for: LinkConfig.baud "
                       "must be positive");
    }
...
Transport::Transport(J2534Library library) noexcept
...
st::Status Transport::open(LinkConfig const &cfg) {
    ...
    if (auto const s = fn.connect(device_id_,
                                    static_cast<u32>(params->protocol),
                                    params->flags, params->baud,
                                    &channel_id_);
        s != Status::NoError) {
    ...
    if (fn.read_version != nullptr) {
        ...
        char dll_buf[80]      = {};
        char api_buf[80]      = {};
        if (fn.read_version(device_id_, firmware_buf, dll_buf, api_buf)
            == Status::NoError) {
...
st::Status Transport::close() {
    ...
    // device is gone).
    if (dc != Status::NoError) return to_st_status(dc, "PassThruDisconnect");
    if (cl != Status::NoError) return to_st_status(cl, "PassThruClose");
    return ok();
}
...
[[nodiscard]] Result<PassThruMsg> build_msg(
    ProtocolId protocol, std::span<std::uint8_t const> payload) {
...
Result<Frame> Transport::send_recv(std::span<std::uint8_t const> payload,
    ...
    if (!msg_r.has_value()) return failure(msg_r.error());
    ...
    // whole budget there.
    auto const t_ms = static_cast<u32>(
        std::max<std::int64_t>(0, timeout.count()));
    if (auto const s = fn.write_msgs(channel_id_, &tx, &tx_count, t_ms);
        s != Status::NoError) {
    ...
    while (true) {
        ...
        auto const  remaining_ms = std::chrono::duration_cast<
            std::chrono::milliseconds>(deadline - now);
        auto const  poll_ms = static_cast<u32>(
            std::max<std::int64_t>(0, remaining_ms.count()));
        auto const  s = fn.read_msgs(channel_id_, &rx, &rx_count, poll_ms);
        if (s == Status::NoError && rx_count >= 1) {
            Frame f;
            f.data.assign(rx.data.data(),
                           rx.data.data() + rx.data_size);
        ...
        if (std::chrono::steady_clock::now() >= deadline) {
            return failure(ErrorCode::TransportTimeout,
                           "j2534::Transport::send_recv: no response "
                           "before deadline");
        }
    }
}
...
st::Status Transport::start_streaming(FrameCallback /*callback*/) {
    return failure(ErrorCode::NotImplemented,
                   "j2534::Transport::start_streaming: streaming "
