...
Result<std::vector<std::uint8_t>> encode_request(
    Opcode op, std::span<std::uint8_t const> payload) {
...
[[nodiscard]] constexpr std::uint8_t infer_request_opcode(
    std::uint8_t response_opcode) noexcept {
...
// 1-byte length too.
[[nodiscard]] constexpr LengthWidth length_width_for_response(
    std::uint8_t response_opcode) noexcept {
...
Result<DecodedFrame> decode_frame(std::span<std::uint8_t const> bytes) {
    ...
    if (bytes.size() < 3) {
        return failure(ErrorCode::ParseError,
                       "obdx::dvi::decode_frame: buffer too short "
    ...
    LengthWidth const lw = is_error
        ? LengthWidth::OneByte
        : length_width_for_response(opcode);
    ...
    if (bytes.size() < 1 + len_size + 1) {  // CMD + LEN + CHK
        return failure(ErrorCode::ParseError,
                       "obdx::dvi::decode_frame: buffer truncated before "
                       "length field");
    }
    ...
    std::size_t const expected =
        1 + len_size + declared_len + 1;  // CMD + LEN + payload + CHK
    if (bytes.size() != expected) {
        std::string msg{
            "obdx::dvi::decode_frame: frame size mismatch (declared "
            "payload "};
    ...
    std::uint8_t const expected_chk =
        checksum(bytes.subspan(0, bytes.size() - 1));
    ...
    if (is_error) {
        ...
        if (declared_len != 2) {
            std::string msg{
                "obdx::dvi::decode_frame: error frame must declare "
