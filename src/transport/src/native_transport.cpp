...
[[nodiscard]] Result<std::vector<std::uint8_t>> read_exactly(IByteChannel &chan, std::size_t n,
...
[[nodiscard]] Result<DecodedFrame> read_native_frame(IByteChannel &chan, milliseconds timeout) {
    ...
    std::size_t const declared_len =
        (static_cast<std::size_t>((*header)[3]) << 8U) | static_cast<std::size_t>((*header)[4]);
    // Remaining: payload (declared_len) + 2-byte CRC.
    auto const deadline = steady_clock::now() + timeout;
    auto const remaining_ms =
        std::chrono::duration_cast<milliseconds>(deadline - steady_clock::now());
    auto rest = read_exactly(chan, declared_len + 2, std::max(milliseconds{0}, remaining_ms));
    if (!rest.has_value())
...
request_response(IByteChannel &chan, std::uint8_t seq, Opcode op,
    ...
    if (auto const *ef = std::get_if<ErrorFrame>(&*resp)) {
        ...
        std::snprintf(buf, sizeof buf,
                      ...
                      static_cast<unsigned>(ef->error_code), static_cast<unsigned>(ef->seq),
    ...
    if (rf == nullptr) {
        return failure(ErrorCode::Unknown, "native::request_response: decoded frame "
    ...
    if (rf->seq != seq) {
        ...
        std::snprintf(buf, sizeof buf,
                      ...
                      static_cast<unsigned>(seq), static_cast<unsigned>(rf->seq));
        return failure(ErrorCode::TransportNack, std::string{buf});
    }
    if (rf->request_op != op) {
        char buf[96];
        std::snprintf(buf, sizeof buf,
                      ...
                      static_cast<unsigned>(op), static_cast<unsigned>(rf->request_op));
        return failure(ErrorCode::TransportNack, std::string{buf});
    }
    return rf->payload;
}
...
Result<ConnectPayload> connect_payload_for(LinkConfig const &cfg) noexcept {
    ...
    case LinkKind::CanIso15765:
        ...
        break;
    case LinkKind::CanFd:
        return failure(ErrorCode::InvalidArgument, "native::connect_payload_for: doc-18 hardware "
                                                   "is classical-CAN-only (no CAN-FD).");
    }
    if (cfg.baud <= 0) {
        return failure(ErrorCode::InvalidArgument, "native::connect_payload_for: LinkConfig.baud "
                                                   "must be positive");
    }
...
st::Status Transport::open(LinkConfig const &cfg) {
    ...
    next_seq_ = 0;
    auto hello = request_response(*channel_, next_seq_++, Opcode::Hello, {}, kHelloTimeout);
    if (!hello.has_value()) {
        next_seq_ = 0;
    ...
    auto connect =
        request_response(*channel_, next_seq_++, Opcode::Connect, connect_payload, kConnectTimeout);
...
st::Status Transport::close() {
    ...
    (void)request_response(*channel_, next_seq_++, Opcode::Disconnect, {}, kCloseTimeout);
    firmware_.clear();
    next_seq_ = 0;
    return ok();
}
...
Result<Frame> Transport::send_recv(std::span<std::uint8_t const> payload,
    ...
    auto tx = request_response(*channel_, next_seq_++, Opcode::SendBytes, payload, tx_budget);
    if (!tx.has_value())
    ...
    auto rx = request_response(*channel_, next_seq_++, Opcode::RecvBytes, rx_args, rx_budget);
    if (!rx.has_value())
...
st::Status Transport::send(std::span<std::uint8_t const> payload) {
    ...
    if (channel_ == nullptr) {
        return failure(ErrorCode::TransportUnavailable, "native::Transport::send: no byte channel");
    }
    auto tx =
        request_response(*channel_, next_seq_++, Opcode::SendBytes, payload, milliseconds{100});
    if (!tx.has_value())
        return failure(tx.error());
    return ok();
}
...
st::Status Transport::start_streaming(FrameCallback /*callback*/) {
    return failure(ErrorCode::NotImplemented, "native::Transport::start_streaming: streaming lands "
