...
// Signal layouts derived from bus captures + CSV exports.
//
...
//   - 12 signals in the wide layout are R²-verified ≥ 0.95 against a
//     real driving sniff (Verification::Verified). Use as ground
//     truth.
//   - Everything else is Verification::Hypothesized — byte position
//     inferred from CSV column order; the catalog name-match still
//     applies, but a future sniff may relocate the position within
//     its DID.
//
// RAM addresses are LF79103P-specific; the same logical signal name
// on a different CID may live at a different address.
using S = SignalStorage;
using V = Verification;

// Narrow layout: 31 signals (all Hypothesized — R²-fit was only run
// against the wide-layout sniff).
constexpr std::array<SignalLayout, 31> kApV1_7_4_2Layout{{
...
// Wide layout: 36 R²-verified signals from the aligned re-fit
// against a real driving sniff.
//
// HISTORY: An earlier draft of this layout shipped 12 "Verified"
// rows from a misaligned OLS fit plus 32 "Hypothesized" rows from
// CSV-column-order inference. A properly time-aligned re-fit
// (RPM anchor R²=0.9999) showed that the misaligned OLS fits were
// compensating for a time-alignment error — most "Verified" scales
// were artifacts of that error, not real transforms. With proper
// alignment, the tester packs raw SSM values directly with canonical
// scales (RPM is x/5.12, not the misaligned 0.21246). The earlier
// "Hypothesized" rows were systematically wrong too, so they are
// dropped entirely.
//
// All entries here are Verified with R²≥0.95 from the aligned re-fit.
// 13 of 49 monitors remain unassigned (Baro, Battery V, DAM,
...
constexpr std::array<SignalLayout, 36> kApV1_7_6_0Layout{{
...
std::span<SignalLayout const> ap_layout(DidLayoutVersion fw) noexcept {
    switch (fw) {
    case DidLayoutVersion::Narrow:
        return v_narrow_layout();
    case DidLayoutVersion::Wide:
        return v_wide_layout();
    }
    return {};
}

std::span<DidPayload const> did_payloads(DidLayoutVersion fw) noexcept {
    switch (fw) {
    case DidLayoutVersion::Narrow:
        return {kDidPayloadsNarrow.data(), kDidPayloadsNarrow.size()};
    case DidLayoutVersion::Wide:
        return {kDidPayloadsWide.data(), kDidPayloadsWide.size()};
    }
    return {};
}

std::size_t total_payload_bytes(DidLayoutVersion fw) noexcept {
    switch (fw) {
    case DidLayoutVersion::Narrow:
        return kTotalPayloadBytesNarrow;
    case DidLayoutVersion::Wide:
        return kTotalPayloadBytesWide;
    }
    return 0;
}
...
find_signal(DidLayoutVersion fw, std::uint16_t did,
    ...
    auto const it =
        std::find_if(layout.begin(), layout.end(),
                     [did, byte_offset](SignalLayout const &s) {
...
find_signal(std::uint16_t did, std::uint8_t byte_offset) noexcept {
    return find_signal(DidLayoutVersion::Wide, did, byte_offset);
}

double decode_signal(SignalLayout const &layout,
    ...
    return static_cast<double>(raw) * layout.scale + layout.offset;
}
