...
[[nodiscard]] std::optional<ChecksumKind> parse_checksum_kind(
...
[[nodiscard]] ChecksumKind checksum_kind_from_pack(
...
class IChecksumRepair {
  public:
    ...
    [[nodiscard]] virtual st::Status repair(
        std::span<std::uint8_t> rom_bytes) noexcept = 0;
...
// pack declare a kind?"
[[nodiscard]] std::unique_ptr<IChecksumRepair> make_checksum_repair(
    ChecksumKind kind);
