...
std::uint8_t mode_of(std::vector<std::uint8_t> const &vs) noexcept {
    ...
    for (std::size_t i = 0; i < vs.size(); ++i) {
        auto const v = vs[i];
        if (first_seen[v] == kSentinel) first_seen[v] = i;
        counts[v]++;
    }
    ...
    for (std::size_t v = 0; v < 256; ++v) {
        ...
        if (counts[v] > best_count
            || (counts[v] == best_count && first_seen[v] < best_first)) {
            best       = v;
            best_count = counts[v];
...
std::vector<std::uint8_t> distinct_values(
    std::vector<std::uint8_t> const &vs) noexcept {
...
ByteClass classify_byte(std::vector<std::uint8_t> const &vs,
    ...
    for (std::size_t i = 1; i < vs.size(); ++i) {
        ...
        if (d == 0) continue;            // duplicate frame, ignore
        ++delta_counts[d];
    }
    ...
    if (considered > 0
        && (static_cast<double>(modal_count) / static_cast<double>(considered))
...
void BaselineModel::finalize(std::chrono::nanoseconds duration) {
    ...
    for (auto &[key, b] : builders_) {
        ...
        ib.dlc        = b.dlc;
        ib.samples    = b.frames;
        ib.freq_hz    = dur_ns > 0.0
                            ? static_cast<double>(b.frames) * 1e9 / dur_ns
                            : 0.0;
    ...
    std::sort(entries_.begin(), entries_.end(),
              [](IdBaseline const &a, IdBaseline const &c) {
                  if (a.bus != c.bus) {
                      return static_cast<std::uint8_t>(a.bus)
                             < static_cast<std::uint8_t>(c.bus);
                  }
...
IdBaseline const *BaselineModel::find(can::BusId    bus,
                                       std::uint32_t can_id) const noexcept {
    for (auto const &e : entries_) {
        if (e.bus == bus && e.can_id == can_id) return &e;
    }
    return nullptr;
}
...
ChangeDetector::ChangeDetector(BaselineModel const &baseline)
    : baseline_{&baseline}, opts_{} {}
...
bool value_in_stable_set(std::vector<std::uint8_t> const &allowed,
...
BaselineModel build_baseline(std::span<can::Frame const> frames,
                              BaselineModel::Options      opts) {
    BaselineModel m{opts};
    for (auto const &f : frames) m.observe(f);
...
std::vector<DiscoveryEvent> detect_changes(
...
// an integer.
bool toml_to_byte_vector(toml::array const *arr,
                         std::vector<std::uint8_t> &out) {
    ...
    for (auto const &el : *arr) {
        ...
        if (!opt.has_value() || *opt < 0 || *opt > 255) return false;
...
void emit_baseline_entry(toml::array &dst, IdBaseline const &ib) {
    ...
    t.insert_or_assign("bus", static_cast<std::int64_t>(
                                  static_cast<std::uint8_t>(ib.bus)));
    ...
    for (std::size_t i = 0; i < ib.dlc; ++i) {
        if (ib.classes[i] != ByteClass::Stable) continue;
        toml::array vs;
...
void emit_event(toml::array &dst, DiscoveryEvent const &e) {
    toml::table t;
    t.insert_or_assign("kind",
                       std::string{e.kind == DiscoveryEvent::Kind::NewId
                                       ? kKindNewId
                                       : kKindChange});
...
Result<IdBaseline> parse_baseline_entry(toml::table const &t) {
    ...
    if (auto const v = t["can_id"].value<std::int64_t>(); v.has_value()) {
        ...
        return failure(ErrorCode::ParseError,
                       "cdb: baseline.id missing can_id");
    }
    ...
    ib.samples  = static_cast<std::size_t>(
        t["samples"].value_or<std::int64_t>(0));
    ...
    auto const tag_bytes = [&](char const *key, ByteClass cls) -> Status {
        ...
        for (auto const &el : *arr) {
            ...
            if (!opt.has_value() || *opt < 0 || *opt >= 8) {
                return failure(ErrorCode::ParseError,
                               std::string{"cdb: bad byte index in "} + key);
            }
    ...
    if (auto const *sv_outer = t["stable_values"].as_array();
        sv_outer != nullptr) {
        std::size_t pos = 0;
        for (std::size_t i = 0; i < 8; ++i) {
            if (ib.classes[i] != ByteClass::Stable) continue;
            if (pos >= sv_outer->size()) break;
            std::vector<std::uint8_t> vs;
            if (!toml_to_byte_vector((*sv_outer)[pos].as_array(), vs)) {
                return failure(ErrorCode::ParseError,
                               "cdb: stable_values: bad byte at position "
                                   + std::to_string(pos));
            }
...
Result<DiscoveryEvent> parse_event_entry(toml::table const &t) {
    ...
    if (*kind_opt == kKindNewId) {
        ...
        return failure(ErrorCode::ParseError,
                       "cdb: unknown event kind '" + *kind_opt + "'");
    }

    e.timestamp_ns =
        t["timestamp_ns"].value_or<std::int64_t>(0);
    if (auto const v = t["can_id"].value<std::int64_t>(); v.has_value()) {
    ...
    if (e.kind == DiscoveryEvent::Kind::NewId) {
        if (auto const *after = t["after"].as_array(); after != nullptr) {
            ...
            for (auto const &el : *after) {
                ...
                if (!opt.has_value() || *opt < 0 || *opt > 255) {
                    return failure(ErrorCode::ParseError,
                                   "cdb: bad after byte in new_id event");
                }
...
Result<std::string> write_cdb_string(Bundle const &b) {
    toml::table root;
    root.insert_or_assign("schema_version",
                          static_cast<std::int64_t>(b.schema_version));
...
Result<Bundle> parse_cdb_string(std::string_view text) {
    ...
    out.schema_version = static_cast<int>(
        root["schema_version"].value_or<std::int64_t>(1));
    ...
    if (auto const *baseline = root["baseline"]["id"].as_array();
        baseline != nullptr) {
        ...
        for (auto const &el : *baseline) {
            ...
            if (tbl == nullptr) {
                return failure(ErrorCode::ParseError,
                               "cdb: baseline.id element is not a table");
            }
            auto ib = parse_baseline_entry(*tbl);
            if (!ib.has_value()) return failure(ib.error());
    ...
    if (auto const *events = root["event"].as_array(); events != nullptr) {
        ...
        for (auto const &el : *events) {
            ...
            if (tbl == nullptr) {
                return failure(ErrorCode::ParseError,
                               "cdb: event element is not a table");
            }
            auto e = parse_event_entry(*tbl);
            if (!e.has_value()) return failure(e.error());
...
Status write_cdb(std::filesystem::path const &path, Bundle const &b) {
    ...
    if (!text.has_value()) return failure(text.error());
    std::ofstream out{path, std::ios::binary};
    if (!out) {
        return failure(ErrorCode::IoFailure,
                       "cdb: cannot open for write: " + path.string());
    }
    out.write(text->data(), static_cast<std::streamsize>(text->size()));
    if (!out) {
        return failure(ErrorCode::IoFailure,
                       "cdb: write failed: " + path.string());
    }
    return ok();
}
