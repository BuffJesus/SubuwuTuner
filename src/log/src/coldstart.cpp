...
TestPhase classify_phase(double rpm, double ect_c, WindowConfig const &cfg) noexcept {
...
double TargetLambdaCurve::sample(double ect_c) const noexcept {
    ...
    if (ect_c <= points.front().first)
        return points.front().second;
    if (ect_c >= points.back().first)
...
ColdStartSnapshot snapshot_from_samples(std::span<double const> samples_row_major,
                                        ...
                                        WindowConfig const &cfg, Methodology const &methodology) {
    ColdStartSnapshot snap{};

    if (pid_count == 0 || samples_row_major.empty() || !has_col(mapping.timestamp_idx) ||
        !has_col(mapping.ect_idx) || !has_col(mapping.rpm_idx) || cfg.ect_bin_width_c <= 0.0) {
        return snap;
    }
    ...
    for (std::size_t row = 0; row < n_rows; ++row) {
        ...
        if (!std::isfinite(ts_raw) || !std::isfinite(ect_c) || !std::isfinite(rpm)) {
        ...
        if (has_col(mapping.observed_lambda_idx)) {
            ps.observed_lambda = samples_row_major[off + mapping.observed_lambda_idx];
        }
        if (has_col(mapping.commanded_lambda_idx)) {
            ps.commanded_lambda = samples_row_major[off + mapping.commanded_lambda_idx];
        }
        ...
        if (has_col(mapping.closed_loop_idx)) {
            double const cl = samples_row_major[off + mapping.closed_loop_idx];
        ...
        if (ect_c < cfg.cold_threshold_c && has_col(mapping.observed_lambda_idx) &&
            std::isfinite(ps.observed_lambda)) {
            std::int64_t const bidx =
                static_cast<std::int64_t>(std::floor(ect_c / cfg.ect_bin_width_c));
            auto &acc = ect_buckets[bidx];
            acc.sum_obs += ps.observed_lambda;
            if (ps.observed_lambda < acc.min_obs)
                acc.min_obs = ps.observed_lambda;
            if (ps.observed_lambda > acc.max_obs)
                acc.max_obs = ps.observed_lambda;
            if (has_col(mapping.commanded_lambda_idx) && std::isfinite(ps.commanded_lambda)) {
    ...
    for (auto const &[bidx, acc] : ect_buckets) {
        if (acc.count < cfg.min_samples_per_bin)
            continue;
        EctBin bin{};
        bin.ect_center_c = (static_cast<double>(bidx) + 0.5) * cfg.ect_bin_width_c;
        bin.observed_lambda_mean = acc.sum_obs / static_cast<double>(acc.count);
        bin.observed_lambda_min = acc.min_obs;
        bin.observed_lambda_max = acc.max_obs;
        if (acc.count_with_cmd > 0) {
            bin.commanded_lambda_mean = acc.sum_cmd / static_cast<double>(acc.count_with_cmd);
        }
        bin.count = acc.count;
        bin.has_lambda = true;
        double const target = methodology.target_lambda_vs_ect.sample(bin.ect_center_c);
        bin.deviation_from_target = std::abs(bin.observed_lambda_mean - target);
        total_dev += bin.deviation_from_target;
        total_dev_count += 1;
        snap.ect_bins.push_back(bin);
    }
    if (total_dev_count > 0) {
        snap.mean_lambda_deviation = total_dev / static_cast<double>(total_dev_count);
    }
...
st::Result<ColdStartSnapshot> snapshot_from_csv(std::string_view csv_path,
                                                PidMapping const &mapping, WindowConfig const &cfg,
                                                Methodology const &methodology) {
    if (!has_col(mapping.timestamp_idx) || !has_col(mapping.ect_idx) || !has_col(mapping.rpm_idx)) {
    ...
    if (!f) {
        return st::failure(st::ErrorCode::FileNotFound,
                           std::string{"coldstart: cannot open '"} + std::string{csv_path} + "'");
    }
    ...
    for (std::size_t row = 1; row < lines.size(); ++row) {
        ...
        if (fields.size() < pid_count) {
            return st::failure(st::ErrorCode::ParseError,
                               std::string{"coldstart: CSV row "} + std::to_string(row + 1) +
