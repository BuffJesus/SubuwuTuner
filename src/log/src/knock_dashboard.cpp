...
inline double row_value(std::span<double const> samples_row_major, std::size_t pid_count,
                        std::size_t row, std::size_t pid_idx) noexcept {
    return samples_row_major[row * pid_count + pid_idx];
}

inline bool sample_passes_gate(std::span<double const> samples_row_major, std::size_t pid_count,
                               std::size_t row, PidMapping const &mapping,
                               WindowConfig const &cfg) noexcept {
    if (!cfg.require_load_gate) {
        return true;
    }
    if (has_pid(mapping.rpm_idx) &&
        row_value(samples_row_major, pid_count, row, mapping.rpm_idx) < cfg.min_rpm) {
        return false;
    }
    if (has_pid(mapping.load_idx) &&
        row_value(samples_row_major, pid_count, row, mapping.load_idx) < cfg.min_load) {
        return false;
    }
    return true;
}
...
KnockSnapshot snapshot_from_samples(std::span<double const> samples_row_major,
    ...
    std::size_t const window_rows =
        (cfg.sample_rate_hz > 0.0)
            ? static_cast<std::size_t>(cfg.sample_rate_hz * cfg.window_seconds)
            : n_rows;
    std::size_t const start_row = (n_rows > window_rows) ? (n_rows - window_rows) : 0;
    ...
    for (std::size_t row = start_row; row < n_rows; ++row) {
        bool const passed = sample_passes_gate(samples_row_major, pid_count, row, mapping, cfg);
        if (!passed) {
        ...
        for (std::uint8_t c = 0; c < snap.cylinder_count; ++c) {
            ...
            if (has_pid(flk_idx)) {
                double const v = row_value(samples_row_major, pid_count, row, flk_idx);
                acc[c].sum_flkc += v;
                acc[c].last_flkc = v;
                ...
                if (snap.per_cyl[c].strip_flkc.size() > snap.strip_capacity) {
                    snap.per_cyl[c].strip_flkc.erase(snap.per_cyl[c].strip_flkc.begin());
                }
                acc[c].count++;
            }
            if (has_pid(fbkc_idx)) {
                double const v = row_value(samples_row_major, pid_count, row, fbkc_idx);
                acc[c].last_fbkc = v;
                if (v < 0.0) {
                    acc[c].event_count++;
                }
                ...
                if (snap.per_cyl[c].strip_fbkc.size() > snap.strip_capacity) {
                    snap.per_cyl[c].strip_fbkc.erase(snap.per_cyl[c].strip_fbkc.begin());
                }
            }
        }
    }
    ...
    for (std::uint8_t c = 0; c < snap.cylinder_count; ++c) {
        ...
        if (acc[c].count > 0) {
            out.mean_flkc_window = acc[c].sum_flkc / static_cast<double>(acc[c].count);
    ...
    if (cyls_with_data > 0) {
        double const all_mean = all_cyl_mean_sum / static_cast<double>(cyls_with_data);
        for (std::uint8_t c = 0; c < snap.cylinder_count; ++c) {
            if (acc[c].count > 0) {
                snap.per_cyl[c].delta_from_cyl_mean = snap.per_cyl[c].mean_flkc_window - all_mean;
            }
        }
    }
...
std::size_t max_pid_index(PidMapping const &mapping) noexcept {
    ...
    for (auto v : mapping.fine_knock_learn)
        consider(v);
    for (auto v : mapping.feedback_knock)
        consider(v);
    for (auto v : mapping.knock_sensor_noise)
        consider(v);
    return lo;
}
...
st::Result<KnockSnapshot> snapshot_from_csv(std::string_view csv_path, PidMapping const &mapping,
                                            WindowConfig const &cfg) {
    std::ifstream f{std::string{csv_path}};
    if (!f) {
        return st::failure(st::ErrorCode::FileNotFound,
                           std::string{"knock-snapshot: cannot open '"} + std::string{csv_path} +
                               "'");
    }
    ...
    for (std::size_t row = 1; row < lines.size(); ++row) {
        ...
        if (fields.size() < pid_count) {
            return st::failure(st::ErrorCode::ParseError,
                               std::string{"knock-snapshot: CSV row "} + std::to_string(row + 1) +
