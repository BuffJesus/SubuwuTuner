...
double interpret_bytes(std::span<std::uint8_t const> bytes, DataType dt) noexcept {
    ...
    case DataType::Uint16Be: {
        auto const v = static_cast<std::uint16_t>((static_cast<std::uint16_t>(b[0]) << 8U) | b[1]);
        return static_cast<double>(v);
    }
    case DataType::Uint16Le: {
        auto const v = static_cast<std::uint16_t>((static_cast<std::uint16_t>(b[1]) << 8U) | b[0]);
        return static_cast<double>(v);
    }
    ...
    case DataType::Uint32Le: {
        auto const v = (static_cast<std::uint32_t>(b[3]) << 24U) |
                       ...
                       (static_cast<std::uint32_t>(b[1]) << 8U) | static_cast<std::uint32_t>(b[0]);
        return static_cast<double>(v);
    }
    ...
    case DataType::Float32Be: {
        auto const bits =
            (static_cast<std::uint32_t>(b[0]) << 24U) | (static_cast<std::uint32_t>(b[1]) << 16U) |
            (static_cast<std::uint32_t>(b[2]) << 8U) | static_cast<std::uint32_t>(b[3]);
        float f = 0.0f;
    ...
    case DataType::Float32Le: {
        auto const bits =
            (static_cast<std::uint32_t>(b[3]) << 24U) | (static_cast<std::uint32_t>(b[2]) << 16U) |
            (static_cast<std::uint32_t>(b[1]) << 8U) | static_cast<std::uint32_t>(b[0]);
        float f = 0.0f;
        std::memcpy(&f, &bits, sizeof(f));
        return static_cast<double>(f);
...
bool LogStream::try_push(std::int64_t timestamp_ns, std::span<double const> values) noexcept {
    assert(values.size() == pid_count_ && "values.size() must equal pid_count()");
...
bool LogStream::try_pop(std::int64_t &out_timestamp_ns, std::span<double> out_values) noexcept {
    assert(out_values.size() == pid_count_ && "out_values.size() must equal pid_count()");
...
Status LogSession::start(std::chrono::milliseconds per_cycle_timeout) {
    if (channels_.empty()) {
        return failure(ErrorCode::InvalidArgument, "LogSession has no channels");
    }
    if (running_.exchange(true, std::memory_order_acq_rel)) {
        // Already running — restore the flag (compare-and-swap style)
        // for an honest no-op.
        return failure(ErrorCode::InvalidArgument, "LogSession is already running");
    }
    ...
    io_thread_ = std::thread([this, per_cycle_timeout] { io_loop(per_cycle_timeout); });
    return ok();
}
...
void LogSession::io_loop(std::chrono::milliseconds timeout) noexcept {
    ...
    while (!stop_requested_.load(std::memory_order_acquire)) {
        ...
        for (std::size_t i = 0; i < channels_.size(); ++i) {
            ...
            std::span<std::uint8_t const> slice{bytes.data() + offset, bytes_per_ch};
            double const raw = interpret_bytes(slice, channels_[i].data_type);
            sample[i] =
                channels_[i].scaling.has_value() ? apply_scaling(raw, *channels_[i].scaling) : raw;
        }
        if (!ok_cycle) {
            io_errors_.fetch_add(1, std::memory_order_relaxed);
            continue;
...
char const *format_value(double v, LogChannel const &ch, char *buf, std::size_t buf_size) {
...
void CsvSink::write_row(std::int64_t timestamp_ns, std::span<double const> values) {
    assert(values.size() == channels_.size() && "CsvSink::write_row size mismatch");
    ...
    if (timestamp_ns < 0 && whole_ms == 0) {
        std::snprintf(ts_buf, sizeof(ts_buf), "-0.%03lld", static_cast<long long>(frac_us));
    } else {
        std::snprintf(ts_buf, sizeof(ts_buf), "%lld.%03lld", static_cast<long long>(whole_ms),
    ...
    char val_buf[32];
    for (std::size_t i = 0; i < channels_.size(); ++i) {
        out_ << ',' << format_value(values[i], channels_[i], val_buf, sizeof(val_buf));
    }
    out_ << '\n';
}
